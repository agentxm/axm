...
import { extensionName, getAppError, handleMcpServersNew } from "axm.sh/specification-harness";
...
export const specification = defineSpecification({
  ...
  methods: ["example"],
  derivedFrom: ["apps/cli/src/root/mcps/new.test.ts"],
  supersedes: [],
  assumptions: [],
  openQuestions: [],
});
...
describe("New MCP server preview purity", () => {
  ...
  it.effect("a previewed scaffold changes no protected state", () =>
    Effect.gen(function* () {
      ...
      expect(workspace.exists("mcps")).toBe(false);
      expect(JSON.stringify(workspace.readSettings())).not.toContain("context");
      ...
      expect(entry?.data).toMatchObject({
        result: {
          ...
          units: [{ label: "@acme/mcps/context", state: "ready" }],
        },
      });
    }),
  );
  ...
  it.effect("the route offers preview and rejects preapproval it cannot use", () =>
    Effect.gen(function* () {
      expect(yield* probeFlag(["mcps", "new"], "--preview")).toBe("accepted");
      expect(yield* probeFlag(["mcps", "new"], "--yes")).toBe("unrecognized");
      expect(yield* probeFlag(["mcps", "new"], "-y")).toBe("unrecognized");
    }),
  );
});
