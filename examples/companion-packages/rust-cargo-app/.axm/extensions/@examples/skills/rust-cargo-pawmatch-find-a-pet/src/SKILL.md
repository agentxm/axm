---
name: rust-cargo-pawmatch-find-a-pet
...
`pawmatch` Rust CLI is available (either as a built binary on `$PATH` or via
`cargo run --` from this repository). The skill drives `pawmatch` to surface
