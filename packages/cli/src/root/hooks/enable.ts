...
export const handleEnableHook = Effect.fn("EnableHook.handle")(function* (args: {
  ...
  if (entry.enabled) {
    yield* emitNoOpResult("hooks.enable", {
  ...
  const { ref, versionRange } = yield* resolveConfiguredHook(args.name, entry.source);
  const plan: Plan = {
    _tag: "Plan",
    name: "Enable hooks",
  ...
  yield* emitPlanResolutionResult("hooks.enable", resolution);
});
