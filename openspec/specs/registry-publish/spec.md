...
#### Scenario: Bare name resolved with profile

- **WHEN** `skills publish code-review` is called and project profile is `@acme`
- **THEN** the extension `@acme/skills/code-review` is published

#### Scenario: Glob pattern publishes multiple skills

- **WHEN** `skills publish "effect-*"` is called and project profile is `@acme`
