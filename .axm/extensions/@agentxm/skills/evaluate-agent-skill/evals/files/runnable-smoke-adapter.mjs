...
const write = (name, value) =>
  writeFileSync(join(resolve(outputPath), name), `${JSON.stringify(value, null, 2)}\n`);
...
if (operation === "capabilities") {
  ...
  write(
    ...
    request.stage === "routing"
      ? {
          ...common,
          selected: request.target.name,
          reason: "Deterministic fixture selection for workflow-mechanics evidence only.",
        }
  ...
  write("grade.json", {
    ...
    assertions: request.assertions.map((assertion) => ({
      assertion,
      result: "pass",
      evidence: "Deterministic runnable-smoke fixture evidence.",
    })),
    detail:
      "This adapter establishes workflow mechanics only and does not support production target, native-routing, release, or approval claims.",
    suite_findings: [],
    usage: { tokens: 0, cost_usd: 0 },
  });
} else {
