...
- **WHEN** user runs `axm commands install @acme/commands/review`
- **THEN** the CLI SHALL resolve the source, materialize the package to `.axm/extensions/`, read `axm-command.json` and `COMMAND.md`, render to each configured agent's commands directory, write a command entry to `settings.json`, and write a lock entry with the `agents` array
