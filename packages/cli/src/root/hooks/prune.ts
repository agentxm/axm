...
const hooksLockArtifact = (scope: JobStepArtifact["scope"]): JobStepArtifact => ({
  ...
  jobs: [
    {
      ...
      steps: staleEntries.map<PlannedJobStep>((name) => ({
        label: name,
        readiness: "ready",
        run: ws.removeHookLock(name).pipe(
          Effect.as({
            result: "success",
            message: `Pruned hooks lock entry ${name}`,
            artifact: hooksLockArtifact(ws.scope),
...
export const handleHookPrune = Effect.fn("HookPrune.handle")(function* (flags: HookPruneFlags) {
  ...
  if (stale.length === 0) {
    yield* emitNoOpOutcome("hooks.prune", {
  ...
  yield* emitAppliedPlanOutcome({
    command: "hooks.prune",
    headline: "Pruned hooks lock entries.",
    resolution,
    suggestions: [],
  });
});
