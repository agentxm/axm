...
/** Helper to set up a temp project with .dart_tool/package_config.json for reader tests. */
const readInTempDir = (
  ...
  Effect.gen(function* () {
    ...
    if (packageConfig !== undefined) {
      ...
      yield* fs.writeFileString(path.join(dartToolDir, "package_config.json"), packageConfig);
    }

    if (targetPubspec !== undefined && packageConfig !== undefined) {
      // Parse the package config to find the rootUri and write the pubspec there
      const config = JSON.parse(packageConfig) as {
