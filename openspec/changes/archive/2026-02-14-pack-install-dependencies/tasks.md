...
- [x] 1.1 Write tests for `buildInstallPlan` with mixed `InstallPackOperation | InstallSkillOperation` operations: pack steps check `lockfile.packs`, skill steps check `lockfile.skills`, already-installed skills marked no-op, pack step before skill steps
- [x] 1.2 Define `PackInstallOp = InstallPackOperation | InstallSkillOperation` union type in `build-plan.ts`
- [x] 1.3 Update `buildInstallPlan` signature to accept `ReadonlyArray<PackInstallOp>` and dispatch on `op.name` for no-op detection
