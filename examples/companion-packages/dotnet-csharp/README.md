...
`AgentXM.Example.TinyFlags.CSharp`.
...
`~/.nuget/packages/agentxm.example.tinyflags.csharp/0.1.0/axm.json` and surface
...
The library lives in `src/AgentXM.Example.TinyFlags.CSharp/TinyFlags.cs` and exposes:
...
declares `pkg:nuget/agentxm.example.tinyflags.csharp@0.1.0` as its companion
package.
