/**
 * One owner of atomic single-file replacement: write a uniquely named temp file in
 ...
 * interruption. Temp names follow `<target>.tmp.<unique>`. That prefix is a cross-package
 * protocol: sweepers in other packages rely on it to clean stale temps.
 *
...
export interface WriteFileAtomicOptions<E> {
  ...
  /** Permissions applied when creating the temporary file. */
    ...
    yield* Effect.gen(function* () {
      yield* (
        typeof content === "string"
          ...
          : fs.writeFile(tempPath, content, { mode: options.mode })
