...
import type { DocsLockEntry, MaterializedFileTarget } from "../lockfile/index.js";
...
import type { ExtensionManager, DocsExtensionTarget } from "../workspace/service-interface.js";
import { WorkspaceMutations } from "../workspace/service-interface.js";
import { resolveConfiguredDocs } from "../workspace/configured-entry-resolution/index.js";
...
import {
  ...
  DOCS_EXTENSION_DIR,
  DocsManifestSchema,
  type DocsExtensionRef,
  type FileInputValue,
  type DocsManifest,
  type GitHostedDocsRef,
  type LocalDocsRef,
  type RegistryDocsRef,
...
export class DocsManager extends ServiceMap.Service<
  DocsManager,
  ExtensionManager<DocsExtensionRef>
>()("@agentxm/client-core/unstable/docs/manager/DocsManager") {}

const decodeDocsManifest = Schema.decodeUnknownEffect(DocsManifestSchema);
...
const gitDocsLockEntry = (
  ref: GitHostedDocsRef,
  now: Date,
  resolvedInputs: Readonly<Record<string, FileInputValue>>,
  materializedTargets: ReadonlyArray<MaterializedFileTarget>,
): DocsLockEntry => {
...
const defaultInputs = (manifest: DocsManifest): Readonly<Record<string, FileInputValue>> => {
...
const markerExtForRef = (ref: DocsExtensionRef, manifest: DocsManifest): string =>
  ref.refType === "registry"
    ? formatFqn({ owner: ref.owner, type: "docs", name: ref.file.name })
    : formatFqn({ owner: manifest.owner, type: "docs", name: manifest.name });

export const DocsManagerLive = Layer.effect(
  DocsManager,
  Effect.gen(function* () {
    ...
    const materializeFromRegistry = (ref: RegistryDocsRef) =>
      provide(
        Effect.gen(function* () {
            ...
            Effect.gen(function* () {
              ...
              yield* fs.makeDirectory(canonicalPath, { recursive: true }).pipe(
                Effect.mapError((error) =>
                  makeAppError({
                    code: "validation",
                    detail: `Failed to create canonical docs package directory: ${canonicalPath}`,
                    cause: error,
                  }),
                ),
              );
              const entries = yield* fs.readDirectory(tmpDir).pipe(
                Effect.mapError((error) =>
                  makeAppError({
                    code: "validation",
                    detail: "Extracted docs package directory could not be read",
                    cause: error,
                  }),
                ),
              );
    ...
    const materializeFromExternal = (ref: GitHostedDocsRef | LocalDocsRef) =>
      provide(
        Effect.gen(function* () {
          const canonicalPath = path.join(
            baseDir,
            ...
            yield* provide(
              copyExtensionDirectory(sourcePath, canonicalPath).pipe(
                Effect.mapError((error) =>
                  makeAppError({
                    code: "validation",
                    detail: `Failed to copy docs package files to ${canonicalPath}`,
                    cause: error,
                  }),
                ),
              ),
    ...
    const materializePackage = (ref: DocsExtensionRef) =>
      Effect.gen(function* () {
        switch (ref.refType) {
          case "registry":
    ...
    const readManifest = (packageRoot: string) =>
      fs.readFileString(path.join(packageRoot, DOCS_MANIFEST_FILENAME)).pipe(
        Effect.flatMap((content) =>
          Effect.try({
            try: (): unknown => JSON.parse(content),
              ...
              makeAppError({
                code: "validation",
                detail: `Failed to parse ${DOCS_MANIFEST_FILENAME}`,
                cause: error,
              }),
          }),
        ),
        Effect.flatMap((content) => decodeDocsManifest(content)),
        Effect.mapError((error) =>
          makeAppError({
            code: "validation",
            detail: `Failed to read ${DOCS_MANIFEST_FILENAME}`,
            cause: error,
          }),
        ),
      );

    const markerExtForLockedTarget = (name: string, lockEntry: DocsLockEntry) =>
      Effect.gen(function* () {
        if (lockEntry.type === "registry") {
          return formatFqn({ owner: lockEntry.owner, type: "docs", name: lockEntry.name });
        }
        const packageRoot = path.join(baseDir, EXTERNAL_EXTENSIONS_DIR, DOCS_EXTENSION_DIR, name);
        const manifest = yield* readManifest(packageRoot).pipe(Effect.option);
        return Option.match(manifest, {
          onNone: () => `file:${name}`,
          onSome: (value) => formatFqn({ owner: value.owner, type: "docs", name: value.name }),
        });
      });
    ...
    const materializeManagedRegion = (args: {
      ...
      readonly entry: Extract<
        DocsManifest["contents"][number],
        { readonly mode: "managed-region" }
      >;
      readonly templateDocs: {
      ...
      Effect.gen(function* () {
        ...
        if (Option.isNone(relative)) {
          return yield* makeAppError({
            code: "validation",
            detail: `docs package target escapes workspace: ${args.entry.target}`,
          });
        }
        ...
        const rendered = yield* provide(
          renderFileContent({
            ...
            templateDocs: args.templateDocs,
            generatedDocs: {
        ...
        yield* fs.makeDirectory(path.dirname(absoluteTarget), { recursive: true }).pipe(
          Effect.mapError((error) =>
            makeAppError({
              code: "internal",
              detail: `Failed to create docs target directory: ${path.dirname(absoluteTarget)}`,
              cause: error,
            }),
          ),
        );
        yield* fs.writeFileString(absoluteTarget, updated).pipe(
          Effect.mapError((error) =>
            makeAppError({
              code: "internal",
              detail: `Failed to write docs package managed region target: ${absoluteTarget}`,
              cause: error,
            }),
          ),
        );
    ...
    const materializeInstall: ExtensionManager<DocsExtensionRef>["materializeInstall"] = Effect.fn(
      "DocsManager.materializeInstall",
    )(function* ({ ref }) {
      const packageRoot = yield* materializePackage(ref);
      const manifest = yield* readManifest(packageRoot);
      const entries = yield* ws.getConfiguredDocsEntries();
      ...
      const templateDocs = { inputs: resolvedInputs, vars, workspace: { root: baseDir } };
      const previous = yield* ws.getLockedDocsEntry(ref.file.name);
    ...
    const buildLockEntry = (ref: DocsExtensionRef): Effect.Effect<DocsLockEntry, never> => {
      ...
      switch (ref.refType) {
        case "registry":
          return Effect.succeed(
            registryDocsLockEntry(ref, now, resolvedInputs, materializedTargets),
          );
        case "git-hosted":
          return Effect.succeed(gitDocsLockEntry(ref, now, resolvedInputs, materializedTargets));
        case "local":
          return Effect.succeed(
            localDocsLockEntry(
              ref,
    ...
    const materializeUninstall: ExtensionManager<DocsExtensionRef>["materializeUninstall"] =
      Effect.fn("DocsManager.materializeUninstall")(function* ({ target }) {
        const locked = yield* ws.getLockedDocsEntry(target.name);
    ...
    return {
      type: "docs",
      isInstalled: ({ target }: { readonly target: DocsExtensionTarget }) =>
        ws.getLockedDocsEntry(target.name).pipe(
          Effect.map((locked) => Option.isSome(locked)),
          Effect.withSpan("DocsManager.isInstalled"),
        ),

      materializeInstall,

      listMaterializable: Effect.fn("DocsManager.listMaterializable")(function* () {
        ...
        const refs = yield* Effect.scoped(
          Effect.forEach(
            ...
            ([name, entry]) =>
              provide(resolveConfiguredDocs(name, entry.source)).pipe(Effect.map(({ ref }) => ref)),
            { concurrency: "unbounded" },
          ),
        );
        return refs;
      ...
      upsertSettingsEntry: Effect.fn("DocsManager.upsertSettingsEntry")(function* ({
        ref,
        versionRange,
      }) {
        ...
        const source =
          ref.refType === "registry"
            ? (() => {
                const fqn = formatFqn({ owner: ref.owner, type: "docs", name: ref.file.name });
        ...
        yield* ws.setDocsEntry(ref.file.name, {
          source,
          enabled: true,
      ...
      removeSettingsEntry: ({ target }: { readonly target: DocsExtensionTarget }) =>
        ws.removeDocsSettings(target.name).pipe(Effect.withSpan("DocsManager.removeSettingsEntry")),

      upsertLockfileEntry: Effect.fn("DocsManager.upsertLockfileEntry")(function* ({
        ref,
        retainedByPack,
      }) {
        const lockEntry = yield* buildLockEntry(ref);
        ...
        if (retainedLockEntry.type === "registry") {
          yield* validateExactResolvedVersion(
            `docs.${ref.file.name}.resolvedVersion`,
            retainedLockEntry.resolvedVersion,
          );
        }
        yield* ws.setDocsLock({
          name: ref.file.name,
      ...
      removeLockfileEntry: ({ target }: { readonly target: DocsExtensionTarget }) =>
        ws.removeDocsLock(target.name).pipe(Effect.withSpan("DocsManager.removeLockfileEntry")),
    } satisfies ExtensionManager<DocsExtensionRef>;
  }),
);
