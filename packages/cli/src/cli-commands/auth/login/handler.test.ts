...
describe("auth login handler", () => {
  ...
  it.effect("displays URL and code for manual entry", () => {
    ...
    return provide(
      Effect.gen(function* () {
        ...
        expect(steps.some((m) => String(m).includes("https://auth.agentxm.ai/device"))).toBe(true);
  ...
  it.effect("shows Login successful when getMe fails", () => {
    const authClientLayer = AuthClientTest({
      initiateDeviceFlow: () =>
        Effect.succeed({
          device_code: "dc-123",
    ...
    return handleLogin().pipe(
      ...
      Effect.map(() => {
        expect(mockLog2.logs.success.some((m) => m.includes("Login successful"))).toBe(true);
      }),
    );
  });
});
