...
const resolvePath = (input: string, cwd: string): string => {
  if (nodePath.isAbsolute(input)) {
    return nodePath.normalize(input);
  }
 ...
 * @example
 * ```typescript
 * import { resolveLocalPath } from "@agentxm/core/experimental/resolution";
 * import { NodeFileSystem } from "@effect/platform-node";
 * import { Effect } from "effect";
 *
 * const program = resolveLocalPath("./my-skill", { cwd: "/home/user" }).pipe(
 *   Effect.provide(NodeFileSystem.layer),
 * );
 *
 * const refs = await Effect.runPromise(program);
 * // [{ type: "skill", source: "path", origin: "/home/user/my-skill", ... }]
 * ```
 *
 * @experimental This API is unstable and may change without notice.
 */
