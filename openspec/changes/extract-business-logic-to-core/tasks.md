...
- [ ] 10.1 Delete `packages/cli/src/sources/index.ts` — pure re-export barrel. Update all ~40 CLI consumers to import directly from `@axm.sh/core/unstable/source-resolution`
- [ ] 10.2 Delete `packages/cli/src/auth/index.ts` — re-export barrel. Update the 2 consumers of `withAuthGuard` to import from `../auth/guard.js` (or appropriate relative path)
- [ ] 10.3 Delete `packages/cli/src/workspace/index.ts` — re-export barrel. Update all ~40 CLI consumers: core types (`Workspace`, `Plan`, `WorkspaceScope`, etc.) import from `@axm.sh/core/unstable/workspace`; CLI-only exports (`layer`) import from `./service.js` or `../workspace/service.js`; `resolvePlan` from `./resolve-plan.js` or `../workspace/resolve-plan.js`
- [ ] 10.4 Delete `packages/cli/src/extensions/registry-ref-builders.ts` and `packages/cli/src/extensions/index.ts` — dead duplicates of core's `extension-managers/registry-ref-builders.ts`. Update any consumers to import from `@axm.sh/core/unstable/extension-managers`
- [ ] 10.5 Delete dead duplicate extension files from CLI: `extensions/skills/manager.ts`, `paths.ts`, `utils.ts`, `reconciliation-adapter.ts`, `manager.test.ts`, `paths.test.ts`, `utils.test.ts` — already exist in core's `extension-managers/skills/`
- [ ] 10.6 Delete dead duplicate extension files: `extensions/packs/manager.ts`, `paths.ts`, `reconciliation-adapter.ts`, `expansion.ts`, `manager.test.ts`, `expansion.test.ts` — already in core
- [ ] 10.7 Delete dead duplicate extension files: `extensions/commands/manager.ts`, `reconciliation-adapter.ts`, `manager.test.ts` — already in core
- [ ] 10.8 Delete dead duplicate extension files: `extensions/mcp-servers/manager.ts`, `reconciliation-adapter.ts`, `manager.test.ts` — already in core
- [ ] 10.9 Delete dead duplicate `extensions/examples.test.ts` — already in core
- [ ] 10.10 Delete dead duplicate workspace files from CLI: `plan.ts`, `apply-plan.ts`, `apply-plan.test.ts`, `classifier.ts`, `classifier.test.ts`, `classifier-records.ts`, `paths.ts`, `paths.test.ts`, `scope.ts`, `reconciliation-types.ts`, `taxonomy-types.ts`, `source-metadata.ts`, `plan-bridge.ts` — already exist in core's `unstable/workspace/`
- [ ] 10.11 Update any remaining imports of deleted files to point to core equivalents
- [ ] 10.12 Run `pnpm typecheck` — fix any errors
- [ ] 10.13 Run `pnpm lint` — fix any errors
- [ ] 10.14 Run `pnpm test` — fix any failures
- [ ] 10.15 Run `pnpm test:e2e` — fix any failures
- [ ] 10.16 Kill any lingering vitest worker processes
...
- [ ] 11.1 Move `packages/cli/src/agents/constants.ts` to `packages/core/src/unstable/agents/constants.ts` — pure path helpers (homedir, XDG config)
- [ ] 11.2 Move `packages/cli/src/agents/mcp-sync.ts` and `mcp-sync.test.ts` to `packages/core/src/unstable/agents/mcp-sync.ts` — pure MCP config sync logic
- [ ] 11.3 Move agent service implementations to `packages/core/src/unstable/agents/`: `claude-code/service.ts`, `cursor/service.ts`, `codex/service.ts`, `gemini-cli/service.ts`, `github-copilot/service.ts`, `opencode/service.ts` — pure `CodingAgent` impls with no CLI deps
- [ ] 11.4 Move `packages/cli/src/agents/repository.ts` and `repository.test.ts` to `packages/core/src/unstable/agents/repository.ts` — `DefaultCodingAgentRepository` implementation. Update to import agent services from core-local paths
- [ ] 11.5 Move `packages/cli/src/agents/coding-agent.ts` and `coding-agent.test.ts` to core — merge with existing core agent types, keeping the `CodingAgentRepository` service tag in core
- [ ] 11.6 Update `packages/core/src/unstable/agents/index.ts` barrel to export all moved modules
- [ ] 11.7 Update all CLI imports of agent modules to `@axm.sh/core/unstable/agents`
- [ ] 11.8 Remove `packages/cli/src/agents/` directory (keep only `index.ts` re-export if needed, or remove entirely)
- [ ] 11.9 Run `pnpm typecheck` — fix any errors
- [ ] 11.10 Run `pnpm lint` — fix any errors
- [ ] 11.11 Run `pnpm test` — fix any failures
- [ ] 11.12 Run `pnpm test:e2e` — fix any failures
- [ ] 11.13 Kill any lingering vitest worker processes
...
- [ ] 12.1 Move `packages/cli/src/workspace/service.ts` and `service.test.ts` to `packages/core/src/unstable/workspace/service.ts` — `WorkspaceContextService` implementation. Remove any CLI-specific imports (should be none after prior phases)
- [ ] 12.2 Move `packages/cli/src/workspace/reconciliation.ts` and `reconciliation.test.ts` to core — adapter registration and lockfile state merging
- [ ] 12.3 Move `packages/cli/src/workspace/builtin-packs.ts` to core — builtin pack materialization logic
- [ ] 12.4 Move `packages/cli/src/workspace/initialization.ts` to core — workspace initialization logic
- [ ] 12.5 Update `packages/core/src/unstable/workspace/index.ts` barrel to export all moved modules including the `layer` (workspace live layer)
- [ ] 12.6 Update all CLI imports of `workspace/service.js` to `@axm.sh/core/unstable/workspace`
- [ ] 12.7 Clean up `packages/cli/src/workspace/` — should retain only `resolve-plan.ts`, `display-plan.ts`, `display-plan.test.ts`, `resolve-plan-architecture.test.ts`, `test-stubs.ts`
- [ ] 12.8 Run `pnpm typecheck` — fix any errors
- [ ] 12.9 Run `pnpm lint` — fix any errors
- [ ] 12.10 Run `pnpm test` — fix any failures
- [ ] 12.11 Run `pnpm test:e2e` — fix any failures
- [ ] 12.12 Kill any lingering vitest worker processes
...
- [ ] 13.1 Move `packages/cli/src/auth/login-interaction.ts` to `packages/core/src/unstable/auth/login-interaction.ts` — platform-aware browser launch and clipboard. Uses `child_process.spawn`, no CliRenderer/CliPrompt
- [ ] 13.2 Move `packages/cli/src/auth/guard.ts` and `guard.test.ts` to `packages/core/src/unstable/auth/guard.ts` — auth flow orchestration using `CliPrompt`, `isNonInteractive`, `runDeviceLogin` (all already in core)
- [ ] 13.3 Update `packages/core/src/unstable/auth/index.ts` barrel to export moved modules
- [ ] 13.4 Update all CLI imports of auth modules to `@axm.sh/core/unstable/auth`
- [ ] 13.5 Remove `packages/cli/src/auth/` directory entirely
- [ ] 13.6 Run `pnpm typecheck` — fix any errors
- [ ] 13.7 Run `pnpm lint` — fix any errors
- [ ] 13.8 Run `pnpm test` — fix any failures
- [ ] 13.9 Run `pnpm test:e2e` — fix any failures
- [ ] 13.10 Kill any lingering vitest worker processes
...
- [ ] 14.1 Verify `packages/cli/src/` retains only: `root/` (command handlers), `workflows/install-command/` and `uninstall-command/`, `workspace/resolve-plan.ts` + `display-plan.ts` + tests + `test-stubs.ts`, `builtin-pack/`, `cli-flags/`, `runtime/`, `telemetry/`, `extensions/*/layers.ts` + `extensions/*/operations/`, entry points (`main.ts`, `app.ts`, `runtime.ts`, `help.ts`, `version.ts`, `output.ts`)
- [ ] 14.2 Verify no dead re-export barrels remain in CLI
- [ ] 14.3 Verify no core source files import from `@axm.sh/cli`
- [ ] 14.4 Run full `pnpm build` — both packages build cleanly
- [ ] 14.5 Run full `pnpm typecheck` — zero errors
- [ ] 14.6 Run full `pnpm lint` — zero errors
- [ ] 14.7 Run full `pnpm test` — all tests pass
- [ ] 14.8 Run full `pnpm test:e2e` — all E2E tests pass
- [ ] 14.9 Kill any lingering vitest worker processes
