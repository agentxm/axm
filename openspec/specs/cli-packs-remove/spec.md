...
The `axm packs remove` command removes extensions from an extension extension pack's manifest.
...
- **AND** `@acme/code-review` is in the extension pack's `skills` section
- **THEN** `@acme/code-review` is removed from the extension extension pack manifest's `skills` section

#### Scenario: Extension not in pack

- **WHEN** user runs `axm packs remove frontend-tools @acme/nonexistent`
- **AND** `@acme/nonexistent` is not in the extension extension pack manifest
- **THEN** the command fails with a `AppError` indicating the extension is not in the extension pack

### Requirement: Glob pattern expansion for remove

When the extension argument contains a glob pattern, the system SHALL expand it against extensions in the extension extension pack manifest.

#### Scenario: Glob matches multiple extensions in pack

- **WHEN** user runs `axm packs remove my-pack "effect-*"`
- **AND** the extension extension pack manifest contains `@acme/effect-basics` and `@acme/effect-streams` in `skills`
...
- **AND** no extensions in the extension extension pack manifest match the pattern
