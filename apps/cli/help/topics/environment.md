...
credentials are never forwarded to any non-default Registry origin. Configure
the durable Registry choice with `defaultRegistry` and `sources` in settings;
see `axm help settings`.
