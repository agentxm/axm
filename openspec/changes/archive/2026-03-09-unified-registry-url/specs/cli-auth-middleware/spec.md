# cli-auth-middleware Specification (Delta)

## Purpose

Replaces single-URL gating with credential-based gating and scoped ambient tokens.
...
#### Scenario: Stored credentials for any registry (ADDED)
...
#### Scenario: AXM_TOKEN scoped to default registry (MODIFIED)
...
#### Scenario: AXM_TOKEN not applied to non-registry hosts (ADDED)
...
#### Scenario: --token flag scoped to default registry (MODIFIED)
...
#### Scenario: AXM_TOKEN works without axm login (ADDED)
...
### Requirement: Non-registry requests pass through (MODIFIED)

Replaces: URL prefix matching against a single configured registry URL.
...
### Requirement: Token resolution split (ADDED)
