...
import { hookReconciliationAdapter } from "./reconciliation-adapter.js";
...
describe("hooks reconciliation adapter", () => {
  let tempDir: string;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "axm-hooks-reconcile-"));
  });
  ...
  it.effect("requires canonical registry metadata instead of reconstructing it from disk", () =>
    Effect.gen(function* () {
      const snapshot = yield* snapshotFor({
        skills: {},
        hooks: {
          baseline: { source: "@acme/hooks/guard@^1", enabled: true },
        },
      });

      expect(Object.keys(snapshot.lockfile.hooks ?? {})).toEqual([]);
  ...
  it.effect("produces no lock entry when no hook is declared", () =>
    Effect.gen(function* () {
      const snapshot = yield* snapshotFor({ skills: {}, hooks: {} });

      expect(snapshot.lockfile.hooks ?? {}).toEqual({});
      expect(snapshot.unresolved).toEqual([]);
    }),
  );
});
