...
import { LOCAL_VERSION, TARGET_VERSION, runUpgradeTrial } from "../testing.js";
...
describe("Upgrade preview purity", () => {
  it.effect("a previewed upgrade touches neither the installer nor its persistent records", () =>
    Effect.gen(function* () {
      const run = yield* runUpgradeTrial({ preview: true });

      expect(run.calls).toEqual([]);
      expect(run.installMetaWrites).toEqual([]);
      expect(run.updateCheckWrites).toEqual([]);
      expect(run.assessment).toMatchObject({
        outcome: "previewed",
  ...
  it.effect("the same request without preview writes the records the preview left alone", () =>
    Effect.gen(function* () {
      const run = yield* runUpgradeTrial();
      ...
      expect(run.assessment).toMatchObject({ disposition: "upgraded" });
    }).pipe(Effect.provide(NodeServices.layer)),
  );
