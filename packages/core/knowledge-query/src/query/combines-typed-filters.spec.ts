...
describe("Conjunctive typed filters", () => {
  it.effect("combines lexical fields and typed metadata with escaped property paths", () => {
    ...
    const request = (omitted?: (typeof exclusions)[number]["filter"]) => ({
    ...
    return workspace
      .provide(
        Effect.gen(function* () {
          const run = Effect.fn("spec.query")(function* (
            omitted?: (typeof exclusions)[number]["filter"],
          ) {
            const result = yield* KnowledgeDiscovery.query(request(omitted));
            if (result.outcome !== "ready") throw new Error("Expected a page");
            return result.page.items.map((item) => item.ref.conceptId);
          });
          expect(yield* run()).toEqual(["session"]);
          ...
          for (const exclusion of exclusions) {
            expect((yield* run(exclusion.filter)).sort()).toEqual(
              ["session", exclusion.concept].sort(),
            );
          }
          const byProperty = yield* KnowledgeDiscovery.query({
            scope: "project",
            properties: ["/producer/team~1name=other"],
          });
          if (byProperty.outcome !== "ready") throw new Error("Expected a page");
          expect(byProperty.page.items.map((item) => item.ref.conceptId)).toEqual(["other"]);
        }),
      )
