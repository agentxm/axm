...
The Zig reader SHALL inspect `axm.json` sidecar files in the Zig package cache at `~/.cache/zig/`. The exact cache structure depends on Zig version, so the reader SHALL perform a best-effort scan. When present and valid, the reader SHALL extract the `recommendedExtensions` array.

#### Scenario: Package with valid axm.json in cache

- **WHEN** `~/.cache/zig/p/<hash>/axm.json` contains `{ "recommendedExtensions": ["@zig/skills/zap@^1.0.0"] }`
- **THEN** the reader SHALL return the extension refs `["@zig/skills/zap@^1.0.0"]`
