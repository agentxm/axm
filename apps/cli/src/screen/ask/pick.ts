...
const toggled = <A>(
  ...
  const pickAll = targets.some((index) => !picked.has(index));
  for (const index of targets) {
    if (pickAll) picked.add(index);
    else picked.delete(index);
  }
  if (pickAll && ask.max !== undefined && picked.size > ask.max) {
    return {
      _tag: "Next",
      state: { ...state, problem: `Pick at most ${count(ask.noun, ask.max)}` },
    };
  }
...
export const reducePick = <A>(ask: PickAsk<A>, state: PickState, key: AskKey): PickAction => {
  ...
  if (key.name === "space" || key.char === " ") {
    ...
    return toggled(ask, state, row._tag === "Group" ? row.shown : [row.index]);
  }
