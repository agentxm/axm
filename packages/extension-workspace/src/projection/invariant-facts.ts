...
>()("@agentxm/extension-workspace/projection/invariant-facts/WorkspaceInvariantFacts") {}

/**
 * Build the live facts service. The application supplies `describeFailure`,
 * its boundary rendering for manager failures, so unavailable-unit facts carry
 * the same message text the boundary would print; the kernel owns no error
 * envelope of its own.
 */
export const makeWorkspaceInvariantFactsLive = (options: {
  readonly describeFailure: (failure: ExtensionManagerFailure) => string;
}) =>
  Layer.effect(
    WorkspaceInvariantFacts,
    Effect.gen(function* () {
        ...
        projectionFacts: Effect.gen(function* () {
          const [ruleResult, hookResult, knowledgeResult] = yield* Effect.all(
            [
              ...
              Effect.result(
                knowledge.projectionPlans().pipe(Effect.flatMap(observeProjectionPlans)),
              ),
            ],
