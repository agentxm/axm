...
export const handleDisableDocs = Effect.fn("DisableDocs.handle")(function* (args: {
  ...
  if (!entry.enabled) {
    yield* emitNoOpResult("docs.disable", {
      planName: "Disable docs",
      message: `docs package "${args.name}" is already disabled`,
    });
    return;
  }
  ...
  const plan: Plan = {
    _tag: "Plan",
    name: "Disable docs",
    description: Option.some(`Disable docs package ${args.name}`),
    jobs: [
      {
        concurrency: 1,
        steps: [
          {
            ...
            run: Effect.gen(function* () {
              yield* ws.updateDocsEntry(args.name, (current) => ({
                ...current,
                enabled: false,
              }));
              yield* docsManager.materializeUninstall({
                target: { type: "docs", name: args.name },
              });
              return {
                result: "success",
  ...
  yield* emitPlanResolutionResult("docs.disable", resolution);
});
...
export const disableCommand = Command.make(
  ...
  ({ name, scope, yes, force, preview }) =>
    handleDisableDocs({ name, yes, force, preview }).pipe(
      withWorkspace(scope),
      withRuntime("docs disable"),
    ),
).pipe(
  withArgvTracking(disableConfig),
  Command.withDescription("Disable a docs package without removing sync-once targets"),
  Command.withExamples([
    {
      command: "axm docs disable workspace-baseline",
      description: "Disable a docs package",
    },
  ]),
);
