...
export const specification = defineSpecification({
  ...
  title: "Skill publish preview reports the admitted skills without distributing anything",
  statement:
    "When skills publish runs in preview mode, it shall report the admitted workspace-authored skills with no execution and shall not upload anything to the target registry or change settings, the lockfile, or authored content.",
  class: "functional",
  role: "experience",
...
describe("Skill publish preview purity", () => {
  ...
  const publishWorkspace = (settings: { readonly skills?: Record<string, unknown> }) => {
  ...
  const preview = (workspace: ReturnType<typeof publishWorkspace>, registryUrl: string) =>
    handleRootPublish(
      publishArgs(registryUrl, {
        types: ["skill"],
        preview: true,
        recoveryCommand: ["skills", "publish"],
      }),
    ).pipe(Effect.provide(makePublishLayer(workspace)));

  it.effect(
    "a previewed skill publish admits the authored skill and changes no protected state",
    () =>
      Effect.gen(function* () {
        const workspace = publishWorkspace({ skills: { review: "workspace" } });
        ...
        expect(entry?.data).toMatchObject({
          ...
          publicationSet: {
            status: "admitted",
            items: [{ type: "skill", name: "review", participation: "publish" }],
          },
          execution: { status: "not-run", outcomes: [{ type: "skill", name: "review" }] },
  ...
  it.effect(
    "a previewed skill publish with nothing authored reports an empty selection and changes nothing",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("the route offers preview and rejects preapproval it cannot use", () =>
    Effect.gen(function* () {
      expect(yield* probeFlag(["skills", "publish"], "--preview")).toBe("accepted");
      expect(yield* probeFlag(["skills", "publish"], "--yes")).toBe("unrecognized");
      expect(yield* probeFlag(["skills", "publish"], "-y")).toBe("unrecognized");
    }),
  );
});
