---
name: ocaml-opam-pawmatch-find-a-pet
description: Help an end user identify and apply for an adoptable pet using the PawMatch OCaml CLI.
---

# Find a Pet with PawMatch

Use this skill when a user wants help finding a pet to adopt and the
`pawmatch` OCaml CLI is available (installed via `opam install` or run via
`dune exec pawmatch --`). The skill drives `pawmatch` to surface
