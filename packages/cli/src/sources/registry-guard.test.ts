    ...
    Effect.gen(function* () {
      ...
      if (call) {
        ...
        if (config?.source === "registry") {
          expect(config.location).not.toContain("~");
          expect(config.location).toMatch(/^\/.*my-registry$/);
        } else {
