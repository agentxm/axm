            ...
            if (namespace === "inputs" && !declaredInputs.has(key)) {
              findings.push(
                advisory(
                  RULE_ID,
                  "error",
                  `Template payload '${payloadPath}' references undeclared input '${key}'. Declare it in context.json inputs or remove the placeholder.`,
                  srcPath(payloadPath),
                ),
              );
