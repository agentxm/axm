 ...
 *
 * Every lock instance owns its own registry of held transitions: an entry's
 * lifetime is exactly the acquisition's scope, and the instance belongs to
 * the `WorkspaceTransactionScope` that created it.
 *
...
/**
 * One invocation's view of transition admission and ownership. `held`
 * consults the instance's own registry, so the workspace transaction can
 * reuse the invocation's hold instead of deadlocking on it and can observe
 * that hold's compromise.
 */
export interface WorkspaceTransitionLock {
  readonly acquire: (
    args: AcquireWorkspaceTransitionArgs,
  ) => Effect.Effect<
    Option.Option<TransitionContention>,
    WorkspaceTransitionAcquireFailure,
    FileSystem.FileSystem | Path.Path | Scope.Scope
  >;
  /** The live hold for a workspace this lock instance acquired, when one exists. */
  readonly held: (workspaceDir: string) => Effect.Effect<Option.Option<HeldWorkspaceTransition>>;
}
...
type HeldRegistry = ReadonlyMap<string, HeldWorkspaceTransition>;

const withHeld = (
  registry: HeldRegistry,
  workspaceDir: string,
  held: HeldWorkspaceTransition,
): HeldRegistry => new Map([...registry, [workspaceDir, held]]);

const withoutHeld = (registry: HeldRegistry, workspaceDir: string): HeldRegistry => {
  const next = new Map(registry);
  next.delete(workspaceDir);
  return next;
};
 ...
 */
const acquireWorkspaceTransitionLock = (
  registry: Ref.Ref<HeldRegistry>,
  args: AcquireWorkspaceTransitionArgs,
): Effect.Effect<
  Option.Option<TransitionContention>,
  WorkspaceTransitionAcquireFailure,
      ...
      const attempt = yield* Effect.uninterruptible(
        Effect.gen(function* () {
          ...
          yield* Effect.addFinalizer(() =>
            Effect.gen(function* () {
              yield* Ref.update(registry, (current) => withoutHeld(current, workspaceDir));
...
/**
 * One process-lock instance over `proper-lockfile`, with its own registry of
 * held transitions. A `WorkspaceTransactionScope` owns exactly one.
 */
export const makeWorkspaceTransitionLock: Effect.Effect<WorkspaceTransitionLock> = Effect.map(
  Ref.make<HeldRegistry>(new Map()),
  (registry): WorkspaceTransitionLock => ({
    acquire: (args) => acquireWorkspaceTransitionLock(registry, args),
    // Keyed by the resolved workspace directory the acquisition recorded.
    held: (workspaceDir) =>
      Effect.map(Ref.get(registry), (current) => Option.fromUndefinedOr(current.get(workspaceDir))),
  }),
);
