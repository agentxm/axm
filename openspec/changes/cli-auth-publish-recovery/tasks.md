...
- [x] 2.1 Create `packages/cli/src/auth/token-resolution.ts` with `resolveToken(registryUrl)` function returning `Effect<Option<TokenSource>, CliError>`. Implements precedence: `AXM_TOKEN` env var → `--token` flag (from args) → `CredentialStore.load()`. Write tests for each precedence level and the empty case.
