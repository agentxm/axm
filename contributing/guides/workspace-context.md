# WorkspaceContext Guide

Use `WorkspaceContext` for workspace reads that need scoped, cached state.

> [Effect Layers Guide](./effect-layers.md) - layer construction and provision

## Scope Model

`WorkspaceContext` exposes `ctx.scope("project")` and `ctx.scope("user")`.
...
Legacy readers re-read disk on every call. `WorkspaceContext` does not. In a
...
const fresh =
  yield *
  makeWorkspaceContext(options).pipe(Effect.flatMap((ctx) => ctx.scope(scope).state.settings));
```
...
```ts
const layer = WorkspaceContextTest(validAll("/workspace", "/home"));
yield *
  Effect.gen(function* () {
