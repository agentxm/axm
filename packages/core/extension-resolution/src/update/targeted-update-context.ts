/**
 * Ownership, authority, and blocker classification for a targeted update.
 *
 * A person naming one extension asks a question the workspace has to answer
 ...
 *
 * The classification is pure over the desired-state graph plus the configured
 * Pack rows; `resolveTargetedUpdateContext` reads those and adds the accepted
 * Pack evidence the fingerprint covers. The fingerprint is the freshness
 * witness a targeted update rechecks under the workspace transition, so a
 * context that changed between planning and apply stops the operation instead
 * of applying a decision the workspace no longer supports.
 *
...
/** Every type a targeted update may name; a Pack is classified as a closure, not a member. */
export type TargetedUpdateTargetType = Exclude<InstallableExtensionType, "pack">;
...
export interface TargetedUpdatePublicContext {
  readonly target: {
    readonly type: TargetedUpdateTargetType;
...
export interface TargetedUpdateTarget {
  readonly type: TargetedUpdatePublicContext["target"]["type"];
  readonly name: string;
  readonly fqn: string;
}

export interface ClassifyTargetedUpdateArgs {
...
/** Everything the classifier reads before it can answer. */
export type TargetedUpdateContextFailure =
  WorkspaceStateReadFailure | WorkspaceSettingsReadFailure | WorkspaceLockfileReadFailure;

/**
 * Read the workspace facts a targeted update is classified against, then
 * classify twice: once to learn which Packs own the target, and once more
 * with those Packs' accepted resolutions folded into the fingerprint.
 */
