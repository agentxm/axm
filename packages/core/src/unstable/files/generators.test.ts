...
describe("file generators", () => {
  ...
  it.effect("renders a markdown table with selected columns", () =>
    run(
      Effect.gen(function* () {
        nodeFs.mkdirSync(nodePath.join(tempDir, "files"), { recursive: true });
        nodeFs.writeFileSync(
          nodePath.join(tempDir, "files", "operations.md"),
          "---\ntitle: Operations\ndescription: dotnet commands\n---\n# heading\n",
        );
        nodeFs.writeFileSync(nodePath.join(tempDir, "files", "axm.md"), "# AXM Extensions\n");
  ...
  it.effect("renders link column as a markdown link", () =>
    run(
      Effect.gen(function* () {
        nodeFs.mkdirSync(nodePath.join(tempDir, "files"), { recursive: true });
        nodeFs.writeFileSync(nodePath.join(tempDir, "files", "guide.md"), "# Guide\n");
        ...
        expect(index).toContain("| [guide.md](files/guide.md) | Guide |");
      }),
    ),
  );
