/**
 * docs package materialization helpers.
 *
...
export interface FileTemplateDocs {
...
export interface RenderFileContentArgs {
  ...
  readonly templateDocs: FileTemplateDocs;
  readonly generatedDocs?:
    | {
...
const resolvePayloadPath = (packageRoot: string, payloadPath: string) =>
  Effect.gen(function* () {
    ...
    if (isEscapingRelative(path, srcRoot, resolved)) {
      return yield* failValidation(`docs package payload path escapes src/: ${payloadPath}`);
    }
    return resolved;
  });
...
const resolveWorkspaceTarget = (workspaceRoot: string, target: string) =>
  Effect.gen(function* () {
    ...
    if (Option.isNone(relative)) {
      return yield* failValidation(`docs package target escapes workspace: ${target}`);
    }
    return {
  ...
  Effect.gen(function* () {
    ...
    return yield* fs.readFileString(resolved).pipe(
      Effect.mapError((error) =>
        makeAppError({
          code: "internal",
          detail: `Failed to read docs payload: ${payloadPath}`,
          cause: error,
        }),
      ),
    );
...
const renderGeneratedContent = (
  generator: FileGeneratorSpec,
  docs: FileTemplateDocs,
  generatedDocs?: RenderFileContentArgs["generatedDocs"],
    ...
    switch (generator.name) {
      ...
      case "toc": {
        ...
        const generatedOwnRegion =
          generatedDocs?.ownRegion !== undefined && Option.isSome(style)
            ? { marker: generatedDocs.ownRegion, style: style.value }
            : undefined;
        const ownRegion =
...
const resolveTemplateExpression = (
  ...
  switch (namespace) {
    ...
    case "workspace": {
      ...
      return Effect.succeed(docs.workspace.root);
    }
    default:
...
export const renderFileTemplate = (
  ...
  Effect.gen(function* () {
    ...
    while (match !== null) {
      ...
      output += yield* resolveTemplateExpression(expression, docs);
...
export const renderFileContent = ({
  ...
  Effect.gen(function* () {
    switch (source.kind) {
        ...
        const chunks = yield* Effect.forEach(
          ...
          (payloadPath) =>
            readPayload(packageRoot, payloadPath).pipe(
              Effect.flatMap((content) => renderFileTemplate(content, templateDocs)),
            ),
          { concurrency: 1 },
        );
        return chunks.join("");
      ...
      case "generated":
        return yield* renderGeneratedContent(source.generator, templateDocs, generatedDocs);
    }
  });
  ...
  Effect.gen(function* () {
    ...
    yield* fs.writeFileString(absoluteTarget, content).pipe(
      Effect.mapError((error) =>
        makeAppError({
          code: "internal",
          detail: `Failed to write docs target: ${absoluteTarget}`,
          cause: error,
        }),
      ),
    );
...
> =>
  Effect.gen(function* () {
    ...
    const content = yield* renderFileContent({
      ...
      source: entry.source,
      templateDocs,
      generatedDocs: { target: entry.target },
    });
