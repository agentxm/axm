...
- `mcp-server-cli`: CLI commands for the full MCP server lifecycle — install, uninstall, list, enable, disable, update, publish, new. Follows existing patterns from skill/pack CLI commands.
...
- **Settings schema** (`packages/cli/src/settings/schema.ts`) — `mcpServers` entry (renamed from `mcp-servers`) upgraded from simple version strings to support enabled/disabled state and resolved env values
