...
| -------- | -------------------------------------- | ------------------------------------------------ |
| registry | `@scope/name` or `@scope/name@version` | `@acme/my-skill`, `@axm.sh/cli@1.0.0`            |
...
The scope segment of a fully qualified name SHALL allow dots (`.`) in addition to word characters and hyphens. The name segment SHALL NOT allow dots. This enables scopes like `@axm.sh` while keeping extension names dot-free.
...
#### Scenario: Registry source with dot in scope

- **WHEN** parsing source string `@axm.sh/manage-skills`
- **THEN** source type is `registry` with scope `@axm.sh`, name `manage-skills`

#### Scenario: FQN validates dot in scope

- **WHEN** validating fully qualified name `@axm.sh/cli`
- **THEN** the `FullyQualifiedNameSchema` SHALL accept it

#### Scenario: FQN rejects dot in name segment

- **WHEN** validating fully qualified name `@acme/my.skill`
- **THEN** the `FullyQualifiedNameSchema` SHALL reject it
