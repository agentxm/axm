...
`../dotnet-csharp-nuget-lib/.axm/extensions/` are designed to operate on.

`pawmatch` is not packable — it exists to demonstrate consumption, not to
publish.

The app also ships its own companion AXM skill,
[`dotnet-csharp-nuget-pawmatch-find-a-pet`](./.axm/extensions/@examples/skills/dotnet-csharp-nuget-pawmatch-find-a-pet/src/SKILL.md),
...
The sibling `../dotnet-csharp-nuget-lib/` is the source of that package but is not
referenced directly.
