...
```
publishVersion(profile, type, name, version, archive, metadata) → Effect<void, AppError, R>
```
...
- Registry refs carry `profile: string`, `name: string`, `version: string`, and `integrity: string`
...
#### Scenario: Registry-sourced ref has profile, name, and version

- **WHEN** `find` returns a ref from a registry source
- **THEN** it has `refType: "registry"` with `profile`, `name`, `version`, and `integrity`
...
#### Scenario: Meta-provider applies profile routing

- **WHEN** `find` is called for `@corp/tool`
- **THEN** the meta-provider iterates profile-matched registries first, then catch-all
...
#### Scenario: fetch extracts profile from ref and delegates to client

- **WHEN** `fetch(source, ref)` is called with a registry-sourced ref
- **THEN** the provider extracts `profile`, `type`, `name`, `version` from the ref's `RegistryRefDetails`, calls `client.getExtension(profile, type, name, version)` to get archive bytes, verifies the SHA-512 integrity, and extracts the zip archive to a temporary directory

#### Scenario: publishExtension delegates to client

- **WHEN** `publishExtension(profile, type, name, version, archive, metadata)` is called
