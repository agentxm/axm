...
`../gem-lib/.axm/extensions/` are designed to operate on.

`pawmatch` is not packable — it exists to demonstrate consumption, not to
publish to RubyGems.

The app also ships its own companion AXM skill,
[`gem-pawmatch-find-a-pet`](./.axm/extensions/@examples/skills/gem-pawmatch-find-a-pet/src/SKILL.md),
...
```bash
ruby -Ilib -I../gem-lib/lib bin/pawmatch browse
ruby -Ilib -I../gem-lib/lib -Itest test/cli_test.rb
```
...
```ruby
gem "agentxm-example-tinyflags", path: "../gem-lib"
```
