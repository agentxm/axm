...
The password-input capability SHALL be provided by `Input.password` from `src/input/`. It SHALL accept `message` with optional `mask` and optional `validate`, and return `Effect<string, CliError | PromptCancelled>`.
