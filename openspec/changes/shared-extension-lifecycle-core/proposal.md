...
- Keep the current plan/job model, but route install/uninstall behavior through shared mutation planning/execution.
- Add per-`ExtensionType` lifecycle drivers (`skill`, `pack`) for type-specific behavior in this change.
...
- Support `skill` variation by `PackagingKind` (`native` vs `non-native`) in driver logic.
- Support `mcp-server` target-scoped operations for workspace and per-agent materialization.
...
- `shared-extension-lifecycle-core`: Shared install/uninstall lifecycle pipeline with centralized workspace state mutations and per-type drivers.
- `command-family-lifecycle-workflows`: Dedicated workflow per command family (install, uninstall) built from shared primitives and reused across `skill`/`mcp-server`/`pack` command members.

### Modified Capabilities

- `cli-skills-install`: Uses install-family workflow + shared lifecycle core with `skill` driver native/non-native branching.
...
- `cli-packs-uninstall`: Uses uninstall-family workflow + shared lifecycle core, including preserve-configured dependency semantics for supported types (`skill`).
...
- Extension operation modules (`packages/cli/src/extensions/`) to expose driver contracts and type-specific materialization.
...
...
11 applied, 1 skipped
by type: skills=11, packs=1
success Done
```

Mutation-level details (for example lockfile upsert vs materialization step) are hidden by default and may be shown only in debug/verbose modes.

## Explicit Non-Goals

- Refactoring publish/update/fork/enable/disable flows.
- Adding `command` or `mcp-server` lifecycle-driver integration in this change (explicit no-op placeholders only).
- Replacing the existing plan/job abstraction.
- Changing user-facing CLI semantics beyond consistency fixes required by shared lifecycle behavior.
