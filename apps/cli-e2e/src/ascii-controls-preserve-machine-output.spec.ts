...
import { ErrorEnvelope, HelpTopicDocument } from "./test-support/machine-documents.js";
...
describe("Machine output ignores human symbol selection", () => {
  it.each(environments)("$label preserves successful and failed documents", async ({ env }) => {
    ...
    try {
      ...
      const result = Schema.decodeUnknownSync(HelpTopicDocument)(
      ...
      const refusal = Schema.decodeUnknownSync(ErrorEnvelope)(JSON.parse(ordinaryFailure.stdout));
