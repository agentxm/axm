...
describe("resolveLatestVersion", () => {
  ...
  it.effect("maps rate limits, upstream failures, and transport failures", () =>
    Effect.gen(function* () {
      ...
      expect((yield* Effect.flip(resolveLatestVersion(unavailable, "1.0.0"))).category).toBe(
        "unavailable",
      );
      expect(
        (yield* Effect.flip(resolveLatestVersion(makeNetworkErrorClient(), "1.0.0"))).category,
      ).toBe("network");
    }),
  );
});
...
describe("resolveExactVersion", () => {
  ...
  it.effect("rejects leading-v and prerelease versions", () =>
    Effect.gen(function* () {
      expect((yield* Effect.flip(resolveExactVersion("v1.2.3", "1.0.0"))).category).toBe(
        "validation",
      );
      expect((yield* Effect.flip(resolveExactVersion("1.2.3-beta.1", "1.0.0"))).category).toBe(
        "validation",
      );
    }),
  );
