...
`WorkspaceContextService` SHALL expose a `resolvePlan(plan: Plan)` method that resolves a plan to its terminal state. `resolvePlan` SHALL read `preview`, `yes`, `nonInteractive`, and `force` from the `CliFlags` service rather than from options provided at workspace construction.
...
- **AND** `resolvePlan` SHALL fail with a `AppError` with code `PLAN_HAS_ERRORS`
...
- **WHEN** `resolvePlan` is called and `CliFlags` has `force: true` and the plan contains `PlannedJobStep` steps with `readiness.status === "error"`
- **THEN** the error messages SHALL be logged as warnings
- **AND** execution SHALL proceed as if no errors were present

#### Scenario: Warning readiness forces confirmation in preview mode

- **WHEN** `resolvePlan` is called and `CliFlags` has `preview: true`, `yes: false`, `nonInteractive: false`, and the plan contains `PlannedJobStep` steps with `readiness.status === "warn"` but no `"error"` steps
- **THEN** the plan SHALL be displayed via `displayPlan`
...
- **WHEN** `resolvePlan` is called and `CliFlags` has `preview: true`, `yes: true`, `nonInteractive: false`, and the plan contains `PlannedJobStep` steps with `readiness.status === "warn"` but no `"error"` steps
- **THEN** the plan SHALL be displayed via `displayPlan`
- **AND** a confirmation prompt SHALL ask "Plan has warnings. Continue anyway?"
...
- **THEN** `resolvePlan` SHALL fail with a `AppError` with code `PLAN_HAS_WARNINGS`
- **AND** `applyPlan` SHALL NOT be called

#### Scenario: Warning readiness forces confirmation in default mode

- **WHEN** `resolvePlan` is called and `CliFlags` has `preview: false`, `nonInteractive: false`, and the plan contains `PlannedJobStep` steps with `readiness.status === "warn"` but no `"error"` steps
...
- **WHEN** `resolvePlan` is called and `CliFlags` has `preview: true`, `yes: false`, `nonInteractive: false`, and no warn/error readiness steps
- **THEN** the plan SHALL be displayed via `displayPlan`
...
- **WHEN** `resolvePlan` is called and `CliFlags` has `preview: true`, `yes: true`, and no warn/error readiness steps
- **THEN** the plan SHALL be displayed via `displayPlan`
...
- **WHEN** `resolvePlan` is called and `CliFlags` has `preview: true`, `yes: false`, `nonInteractive: true`, and no warn/error readiness steps
- **THEN** the plan SHALL be displayed via `displayPlan`
- **AND** `applyPlan` SHALL NOT be called
...
- **WHEN** `resolvePlan` is called and `CliFlags` has `preview: false` and no warn/error readiness steps
