...
A context package materializes one or more shared files into a workspace.
...
Context packages live in `./.axm/extensions/<@owner>/context/<package-name>`.

## context.json

[`context.json`](https://axm.sh/schemas/context.schema.json)

Run `axm help context-schema` to print the raw JSON Schema.

## Package layout

```text
.axm/extensions/@acme/context/workspace-baseline/
  context.json
  src/
    AGENTS.md
    README.md
...
`context.json` is the package manifest. It can declare one or more `contents`
...
live under `context.<name>.inputs`.
...
context package entry for richer configurations.

## Authoring and editing files

Run `axm context new <name>` to scaffold a managed context package.

Edit package source files under `.axm/extensions/.../context/<name>/src/`, then
...
Install with `axm context install @owner/context/<name>`. Publish with
`axm context publish @owner/context/<name>`.

## Recommended packs

Name the pack(s) your context package is designed to ship with in `context.json` `recommendedPacks`. Use the bare pack reference — do not include a version range:
...
- `axm context --help` - full context subcommand surface
- `axm help context-schema` - exact manifest fields
- `axm help settings` - workspace vars and context inputs
- `axm help packs` - bundling context packages with packs
