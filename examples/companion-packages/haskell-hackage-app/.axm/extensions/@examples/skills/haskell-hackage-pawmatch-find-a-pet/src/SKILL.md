---
name: haskell-hackage-pawmatch-find-a-pet
description: Help an end user identify and apply for an adoptable pet using the PawMatch Haskell CLI.
---

# Find a Pet with PawMatch

Use this skill when a user wants help finding a pet to adopt and the
`pawmatch` Haskell CLI is available (installed via `cabal install` or run
via `cabal run pawmatch --`). The skill drives `pawmatch` to surface
