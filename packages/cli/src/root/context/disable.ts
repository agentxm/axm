...
export const handleDisableContext = Effect.fn("DisableContext.handle")(function* (args: {
  ...
  if (!entry.enabled) {
    yield* emitNoOpResult("context.disable", {
      planName: "Disable context",
      message: `context package "${args.name}" is already disabled`,
    });
    return;
  }
  ...
  const plan: Plan = {
    ...
    description: Option.some(`Disable context package ${args.name}`),
    jobs: [
      {
        concurrency: 1,
        steps: [
          {
            ...
            run: Effect.gen(function* () {
              yield* ws.updateContextEntry(args.name, (current) => ({
                ...current,
                enabled: false,
              }));
              yield* contextManager.materializeUninstall({
                target: { type: "context", name: args.name },
              });
              return {
                result: "success",
  ...
  yield* emitPlanResolutionResult("context.disable", resolution);
});
...
export const disableCommand = Command.make(
  ...
  ({ name, scope, yes, force, preview }) =>
    handleDisableContext({ name, yes, force, preview }).pipe(
      withWorkspace(scope),
      withRuntime("context disable"),
    ),
).pipe(
  withArgvTracking(disableConfig),
  Command.withDescription("Disable a context package without removing sync-once targets"),
  Command.withExamples([
    {
      command: "axm context disable workspace-baseline",
      description: "Disable a context package",
    },
  ]),
);
