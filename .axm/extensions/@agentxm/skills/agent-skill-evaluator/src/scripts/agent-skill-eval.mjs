...
const ROUTING_MODES = new Set([
  "native-routing",
  "host-simulated-routing",
  "catalog-classification-proxy",
]);
...
class EvalError extends Error {
  constructor(
    message,
    { code = "evaluation-error", exitCode = 2, details = null, disposition = null } = {},
  ) {
    super(message);
    this.name = "EvalError";
    this.code = code;
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < rest.length; index += 1) {
    ...
    if (!token.startsWith("--"))
      fail(`Unexpected argument: ${token}`, { code: "invalid-arguments" });
    ...
    if (value === undefined || value.startsWith("--"))
      fail(`Missing value for ${token}`, { code: "invalid-arguments" });
    index += 1;
...
function readJsonBounded(path, maximumBytes) {
  ...
  if (size > maximumBytes)
    fail(`${path}: output exceeds ${maximumBytes} bytes`, { code: "output-budget-exceeded" });
...
function redactPrivatePaths(value, privateRoots = []) {
  let redacted = value;
  for (const root of [...new Set(privateRoots.filter(Boolean).map((entry) => resolve(entry)))].sort(
    (left, right) => right.length - left.length,
  )) {
    redacted = redacted.replace(new RegExp(escapeRegExp(root), "g"), "<private-root>");
  }
  redacted = redacted.replace(
    new RegExp(`${escapeRegExp(macosUsersRoot)}(?!<redacted>)${privatePathSegmentPattern}`, "g"),
    `${macosUsersRoot}<redacted>`,
  );
  redacted = redacted.replace(/\/home\/(?!<redacted>)[^/\\\s"'`]+/g, "/home/<redacted>");
  redacted = redacted.replace(
    /[A-Za-z]:\\\\Users\\\\(?!<redacted>)[^\\\s"'`]+/g,
    "C:\\\\Users\\\\<redacted>",
  );
  redacted = redacted.replace(
    /[A-Za-z]:\\Users\\(?!<redacted>)[^\\\s"'`]+/g,
...
function sanitizeEvidenceTree(root, privateRoots = []) {
  ...
  for (const path of walkFiles(root)) {
    ...
    const unsafePosixUserPath = new RegExp(
      `(?:${escapeRegExp(macosUsersRoot)}|/home/)(?!<redacted>)${privatePathSegmentPattern}`,
    );
    if (
      unsafePosixUserPath.test(sanitized) ||
      /[A-Za-z]:\\\\Users\\\\(?!<redacted>)[^\\\s"'`]+/.test(sanitized) ||
      /[A-Za-z]:\\Users\\(?!<redacted>)[^\\\s"'`]+/.test(sanitized)
    ) {
      fail(`Generated evidence retained a private user path in ${path}`, {
        code: "unsafe-generated-evidence",
      });
    }
  }
...
function pathIdentity(path) {
  ...
  if (!existsSync(absolute))
    fail(`Identity target does not exist: ${absolute}`, { code: "missing-identity-target" });
...
function runnerIdentity() {
  const paths = [
    ...walkFiles(join(skillSourceRoot, "scripts")),
    ...walkFiles(join(skillSourceRoot, "schemas")),
  ];
...
function safeId(value, label = "identifier") {
  if (typeof value !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(value))
    fail(`Unsafe ${label}: ${value}`, { code: "unsafe-identifier" });
  return value;
}
...
function validateTargetPackage(packageRoot, findings) {
  ...
  if (!existsSync(manifestPath)) {
    findings.push(`${packageRoot}: skill.json is missing`);
    return null;
  }
  if (
    relative(resolve(packageRoot), manifestPath) !==
    relative(realpathSync(packageRoot), realpathSync(manifestPath))
  )
    findings.push(`${manifestPath}: manifest may not be a symlink`);
  if (!existsSync(join(sourceRoot, "SKILL.md")))
    findings.push(`${packageRoot}: src/SKILL.md is missing`);
  if (
    existsSync(sourceRoot) &&
    relative(resolve(packageRoot), sourceRoot) !==
      relative(realpathSync(packageRoot), realpathSync(sourceRoot))
  )
    findings.push(`${sourceRoot}: runtime payload root may not be a symlink`);
  let manifest;
  try {
  ...
  if (typeof manifest.owner !== "string" || !/^@[A-Za-z0-9][A-Za-z0-9._-]*$/.test(manifest.owner))
    findings.push(`${manifestPath}: owner must be a safe scoped identifier`);
  if (typeof manifest.name !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(manifest.name))
    findings.push(`${manifestPath}: name must be a safe identifier`);
  for (const path of walkSymlinks(sourceRoot))
    findings.push(`${path}: evaluated runtime payload symlinks are not permitted`);
  return manifest;
}
...
function validateSuite(packageRoot, findings) {
  ...
  if (suite.evaluation_contract !== "evals/evaluation-contract.json")
    findings.push(
      `${suitePath}: evaluation_contract must reference evals/evaluation-contract.json`,
    );
  ...
  for (const item of suite.evals) {
    ...
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(id))
      findings.push(`${suitePath}: every case id must be safe for evidence paths`);
    ids.add(id);
    ...
    if (typeof item.prompt !== "string" || item.prompt.length === 0)
      findings.push(`${suitePath} case ${id}: prompt is required`);
    if (
      !Array.isArray(item.assertions) ||
      item.assertions.length === 0 ||
      item.assertions.some((value) => typeof value !== "string" || !value)
    )
      findings.push(`${suitePath} case ${id}: assertions are required`);
    if (
      item.deterministic_assertions !== undefined &&
      !Array.isArray(item.deterministic_assertions)
    )
      findings.push(`${suitePath} case ${id}: deterministic_assertions must be an array`);
    for (const assertion of item.deterministic_assertions ?? []) {
      if (assertion?.kind !== "forbid-target-execution")
        findings.push(
          `${suitePath} case ${id}: unsupported deterministic assertion kind ${assertion?.kind}`,
        );
      if (
        typeof assertion?.assertion !== "string" ||
        !item.assertions?.includes(assertion.assertion)
      )
        findings.push(
          `${suitePath} case ${id}: deterministic assertion must name an exact case assertion`,
        );
      if (
        !Array.isArray(assertion?.targets) ||
        assertion.targets.length === 0 ||
        assertion.targets.some((target) => !isSafeWorkspacePath(target))
      )
        findings.push(
          `${suitePath} case ${id}: deterministic assertion targets must be safe workspace paths`,
        );
      if (
        !Array.isArray(assertion?.launchers) ||
        assertion.launchers.length === 0 ||
        assertion.launchers.some(
          (launcher) => typeof launcher !== "string" || !/^[A-Za-z0-9._+-]+$/.test(launcher),
        )
      )
        findings.push(
          `${suitePath} case ${id}: deterministic assertion launchers must be safe command names`,
        );
    }
    if (item.stage === "routing") {
      if (item.files?.length)
        findings.push(`${suitePath} case ${id}: routing cases cannot expose fixtures`);
      ...
      if (!(
        ...
        findings.push(
          `${suitePath} case ${id}: expected_selection must be a string or non-empty string array`,
        );
      if (!Array.isArray(item.catalog_neighbors))
        findings.push(`${suitePath} case ${id}: catalog_neighbors is required`);
    }
    ...
    for (const fixture of item.files ?? []) {
      ...
      if (typeof source !== "string") {
        findings.push(
          `${suitePath} case ${id}: every fixture must be a path or source/target mapping`,
        );
        continue;
      }
      const fixturePath = resolve(packageRoot, source);
      if (
        !isWithin(fixtureRoot, fixturePath) ||
        fixturePath === resolve(fixtureRoot) ||
        !existsSync(fixturePath)
      )
        findings.push(`${suitePath} case ${id}: missing or unsafe fixture ${source}`);
      else if (!isWithin(realpathSync(fixtureRoot), realpathSync(fixturePath)))
        findings.push(
          `${suitePath} case ${id}: fixture escapes evals/files through a symlink ${source}`,
        );
      if (target !== null) {
        if (!isSafeWorkspacePath(target))
          findings.push(`${suitePath} case ${id}: unsafe fixture target ${target}`);
        ...
        if (target === activeTarget || target.startsWith(`${activeTarget}/`))
          findings.push(
            `${suitePath} case ${id}: fixture target may not overwrite the evaluated skill`,
          );
        if (fixtureTargets.has(target))
          findings.push(`${suitePath} case ${id}: duplicate fixture target ${target}`);
        fixtureTargets.add(target);
      }
    }
    for (const artifactPath of item.artifact_paths ?? [])
      if (!isSafeWorkspacePath(artifactPath))
        findings.push(`${suitePath} case ${id}: unsafe artifact path ${artifactPath}`);
  }
  ...
  if (!SUPPORTED_CONTRACT_VERSIONS.has(contract.contract_version))
    findings.push(
      `${contractPath}: contract_version must be one of ${[...SUPPORTED_CONTRACT_VERSIONS].join(", ")}`,
    );
  ...
  if (resultFields.length === 0)
    findings.push(`${contractPath}: required_result_fields must not be empty`);
  for (const field of resultFields) {
    const root = typeof field === "string" ? field.split(".")[0] : "";
    if (
      typeof field !== "string" ||
      !field ||
      (field.includes(".") ? !allowedRoots.has(root) : !allowedSingletons.has(field))
    )
      findings.push(
        `${contractPath}: required result field must be an absolute run path: ${field}`,
      );
  }
  ...
  if (contract.contract_version === CURRENT_CONTRACT_VERSION) {
    const analysis =
      contract.analysis &&
      typeof contract.analysis === "object" &&
      !Array.isArray(contract.analysis)
        ? contract.analysis
        : {};
    if (analysis !== contract.analysis)
      findings.push(`${contractPath}: analysis must be an object`);
    for (const field of REQUIRED_MECHANISM_RESULT_FIELDS)
      if (!resultFields.includes(field))
        findings.push(`${contractPath}: required mechanism result field ${field} is missing`);
    if ("critical_case_ids" in analysis)
      findings.push(
        `${contractPath}: analysis.critical_case_ids is legacy; contract 3.0.0 must map critical gates to exact assertions`,
      );
    const gates = contract.grading?.critical_gates;
    if (
      !Array.isArray(gates) ||
      gates.length === 0 ||
      gates.some((gate) => typeof gate !== "string" || !gate) ||
      new Set(gates).size !== gates.length
    )
      findings.push(
        `${contractPath}: grading.critical_gates must be a non-empty array of unique strings`,
      );
    const mappings = analysis.critical_assertions;
    if (!Array.isArray(mappings) || mappings.length === 0)
      findings.push(
        `${contractPath}: every critical gate must map to a case assertion in analysis.critical_assertions`,
      );
    else {
      ...
      for (const mapping of mappings) {
        ...
        if (!Array.isArray(gates) || !gates.includes(gate))
          findings.push(
            `${contractPath}: critical assertion mapping must name an exact critical gate: ${gate}`,
          );
        else mappedGates.add(gate);
        const activeCase = casesById.get(caseId);
        if (!activeCase)
          findings.push(
            `${contractPath}: critical assertion mapping references unknown case ${caseId}`,
          );
        else if (!activeCase.assertions.includes(assertion))
          findings.push(
            `${contractPath}: critical assertion mapping for case ${caseId} must name an exact assertion`,
          );
      }
      for (const gate of Array.isArray(gates) ? gates : [])
        if (!mappedGates.has(gate))
          findings.push(
            `${contractPath}: every critical gate must map to a case assertion; unmapped gate: ${gate}`,
          );
    }
  }
  ...
  for (const stage of STAGES)
    if (!declaredStages.has(stage))
      findings.push(`${contractPath}: scope.stages must include ${stage}`);
  const outcomes = new Set(contract.evidence?.outcomes ?? []);
  for (const outcome of OUTCOMES)
    if (!outcomes.has(outcome))
      findings.push(`${contractPath}: evidence.outcomes must include ${outcome}`);
  if (
    contract.estimand?.type !== "fixed-suite" ||
    typeof contract.estimand?.sampling_unit !== "string" ||
    typeof contract.estimand?.uncertainty !== "string"
  )
    findings.push(
      `${contractPath}: estimand must declare a fixed-suite sampling unit and uncertainty method`,
    );
  if (
    !Array.isArray(contract.lifecycle?.terminal_states) ||
    !["complete", "failed", "canceled"].every((state) =>
      contract.lifecycle.terminal_states.includes(state),
    )
  )
    findings.push(
      `${contractPath}: lifecycle must declare complete, failed, and canceled terminal states`,
    );
...
function catalogForCase(root, packageRoot, item) {
  ...
  const owners = existsSync(ownersRoot)
    ? [
        manifest.owner,
  ...
  for (const name of names) {
    ...
    const matches = eligibleOwners
      .map((owner) => ({ owner, path: join(ownersRoot, owner, "skills", name, "src", "SKILL.md") }))
      .filter((entry) => existsSync(entry.path));
    if (matches.length === 0)
      fail(`Routing catalog entry is unavailable: ${name}`, { code: "missing-catalog-entry" });
    if (matches.length > 1)
      fail(`Routing catalog entry is ambiguous across owners: ${name}`, {
        code: "ambiguous-catalog-entry",
      });
    const match = matches[0];
    entries.push({
      name,
      owner: match.owner,
      description: parseFrontmatterDescription(match.path),
      source_path: relativePortable(root, match.path, "Catalog entry"),
      content_identity: pathIdentity(match.path),
    });
  }
  return {
    entries,
    content_identity: jsonIdentity(entries),
    trial: entries.map(({ name, description }) => ({ name, description })),
  };
}

function portableEnvironment(allowedNames, extra = {}) {
  const env = {};
  for (const name of [...PORTABLE_ENV, ...allowedNames])
...
function runProcess(command, args, env, timeoutMs, maximumOutputBytes) {
  return new Promise((resolvePromise) => {
    ...
    capture(child.stdout, (value) => {
      stdout += value;
    });
    capture(child.stderr, (value) => {
      stderr += value;
    });
    let killTimer;
...
function validateCapabilities(value, path) {
  ...
  if (value?.schema_version !== PROTOCOL_VERSION)
    problems.push(`schema_version must be ${PROTOCOL_VERSION}`);
  if (
    !Array.isArray(value?.protocol_versions) ||
    !value.protocol_versions.includes(PROTOCOL_VERSION)
  )
    problems.push(`protocol_versions must include ${PROTOCOL_VERSION}`);
  if (!value?.adapter?.name || !value?.adapter?.version)
    problems.push("adapter name and version are required");
  if (!Array.isArray(value?.operations) || !value.operations.includes("trial"))
    problems.push("trial operation is required");
  if (!Array.isArray(value?.stages) || value.stages.some((stage) => !STAGES.has(stage)))
    problems.push("stages must contain supported evaluation stages");
  if (!new Set([...ROUTING_MODES, "unsupported"]).has(value?.routing_mode))
    problems.push("routing_mode is invalid");
  if (!Array.isArray(value?.sandbox_modes)) problems.push("sandbox_modes must be an array");
  if (!new Set(["declared", "observed", "verified", "enforced"]).has(value?.sandbox_status))
    problems.push("sandbox_status is invalid");
  ...
  if (
    !value?.network ||
    !new Set(["denied", "allowlist", "unobserved"]).has(value.network.mode) ||
    !new Set(["declared", "observed", "verified", "enforced"]).has(value.network.status)
  )
    problems.push("network mode or status is invalid");
  if (!new Set(["verified", "declared", "not-verified"]).has(value?.credential_isolation))
    problems.push("credential_isolation is invalid");
  if (
    !value?.lifecycle ||
    !new Set(["process-tree-signal", "adapter-owned", "unsupported"]).has(
      value.lifecycle.cancellation,
    )
  )
    problems.push("lifecycle.cancellation is invalid");
  if (
    !value?.lifecycle ||
    !new Set(["runner-reinvocation", "adapter-owned", "unsupported"]).has(value.lifecycle.retry)
  )
    problems.push("lifecycle.retry is invalid");
  if (
    !value?.lifecycle ||
    !new Set(["stateless", "adapter-checkpoint", "unsupported"]).has(value.lifecycle.resume)
  )
    problems.push("lifecycle.resume is invalid");
  if (problems.length)
    fail(`${path}: ${problems.join("; ")}`, { code: "invalid-adapter-capabilities" });
}
...
async function queryCapabilities(adapter, args, root) {
  ...
  try {
    ...
    const result = await runProcess(
      invocation.command,
      invocation.args,
      portableEnvironment(args.allowEnv, { EVAL_REPO_ROOT: root, EVAL_MODEL: args.model ?? "" }),
      Number(args.preflightTimeoutMs ?? 30000),
      Number(args.maxOutputBytes ?? 1048576),
    );
    const path = join(operationRoot, "capabilities.json");
    if (result.code !== 0 || result.timedOut || result.exceeded || !existsSync(path))
      fail(
        `Adapter capability preflight failed: exit=${result.code} timeout=${result.timedOut} output_exceeded=${result.exceeded}`,
        { code: "adapter-preflight-failed", details: { stderr: result.stderr.slice(0, 4096) } },
      );
...
function validateTrialResponse(item, response) {
  if (response?.schema_version !== PROTOCOL_VERSION)
    fail(`response schema_version must be ${PROTOCOL_VERSION}`, {
      code: "invalid-adapter-response",
    });
  if (
    !Array.isArray(response.side_effects) ||
    typeof response.observations !== "object" ||
    response.observations === null ||
    typeof response.usage !== "object" ||
    response.usage === null
  )
    fail("response must include side_effects, observations, and usage", {
      code: "invalid-adapter-response",
    });
  if (
    response.side_effects.some((value) => typeof value !== "string") ||
    (response.artifacts !== undefined && !Array.isArray(response.artifacts))
  )
    fail("response side_effects or artifacts are invalid", { code: "invalid-adapter-response" });
  ...
  if (
    ...
    fail("response usage must contain non-negative tokens and cost_usd or null", {
      code: "invalid-adapter-response",
    });
  if (item.stage === "routing") {
    ...
    if (
      ...
      fail("routing response must include selected and reason", {
        code: "invalid-adapter-response",
      });
  } else if (typeof response.final_response !== "string")
    fail("execution response must include final_response", { code: "invalid-adapter-response" });
}

function routingGrade(item, response) {
  const expected = Array.isArray(item.expected_selection)
    ? item.expected_selection
    : [item.expected_selection];
...
function validateGradeResponse(grade) {
  if (grade?.schema_version !== PROTOCOL_VERSION)
    fail(`grade schema_version must be ${PROTOCOL_VERSION}`, { code: "invalid-grader-response" });
  if (!OUTCOMES.has(grade.outcome))
    fail("grade outcome is invalid", { code: "invalid-grader-response" });
  if (!(grade.failure_class === null || typeof grade.failure_class === "string"))
    fail("grade failure_class must be a string or null", { code: "invalid-grader-response" });
  if (
    !Array.isArray(grade.assertions) ||
    typeof grade.detail !== "string" ||
    !Array.isArray(grade.suite_findings)
  )
    fail("grade must include assertions, detail, and suite_findings", {
      code: "invalid-grader-response",
    });
  if (
    grade.assertions.some(
      (entry) =>
        typeof entry?.assertion !== "string" ||
        !new Set(["pass", "fail", "unknown"]).has(entry?.result) ||
        typeof entry?.evidence !== "string",
    ) ||
    grade.suite_findings.some((entry) => typeof entry !== "string")
  )
    fail("grade assertion or suite-finding evidence is invalid", {
      code: "invalid-grader-response",
    });
  ...
  if (
    ...
    fail("grade usage must contain non-negative tokens and cost_usd or null", {
      code: "invalid-grader-response",
    });
}
...
function shellPayload(command) {
  ...
  for (let depth = 0; depth < 3; depth += 1) {
    const match = payload.match(
      /^\s*(?:\S*\/)?(?:sh|bash|zsh)\s+-(?:c|lc|cl)\s+(['"])([\s\S]*)\1\s*$/,
    );
    if (!match) break;
    payload = match[2];
  }
  ...
  for (const segment of shellPayload(command).split(/&&|\|\||[;|]/)) {
    ...
    for (const target of assertion.targets) {
      const targetIndex = tokens.findIndex(
        (token) => token === target || token.endsWith(`/${target}`),
      );
      if (targetIndex < 0) continue;
      const commandName = basename(tokens[0] ?? "");
      if (targetIndex === 0 || new Set([".", "source"]).has(commandName)) return true;
      if (
        tokens.slice(0, targetIndex).some((token) => assertion.launchers.includes(basename(token)))
      )
        return true;
    }
  }
  ...
  for (const assertion of item.deterministic_assertions ?? []) {
    ...
    if (!Array.isArray(toolCalls)) {
      results.set(assertion.assertion, {
        ...
        result: "unknown",
        evidence: "Structured tool-call evidence was unavailable.",
      });
      continue;
    }
    const attempted = toolCalls.filter(
      (entry) =>
        entry?.type === "command_execution" && commandExecutesTarget(entry.command, assertion),
    );
    results.set(assertion.assertion, {
      assertion: assertion.assertion,
      result: attempted.length ? "fail" : "pass",
      evidence: attempted.length
        ? `Structured tool-call evidence recorded ${attempted.length} forbidden target execution attempt(s).`
        : `Structured tool-call evidence recorded no forbidden target execution across ${toolCalls.length} command observation(s).`,
    });
  }
  return results;
}
...
function normalizeGrade(item, grade, response) {
  ...
  if (grade?.outcome === "harness-error")
    return {
      ...grade,
      assertions: item.assertions.map((assertion) => ({
        assertion,
      ...
      return {
        ...
        evidence:
          matches.length === 0
            ...
            : "Grader returned duplicate or invalid results for this required assertion",
      };
    return matches[0];
  });
  ...
  const normalized = {
    ...
    failure_class:
      outcome === "pass"
        ? null
        : (grade?.failure_class ??
          (outcome === "fail" ? "assertion-failure" : "incomplete-grading")),
    assertions,
    detail: typeof grade?.detail === "string" ? grade.detail : "",
    suite_findings: Array.isArray(grade?.suite_findings) ? grade.suite_findings : [],
  };
  if ([...deterministic.values()].some((entry) => entry.result === "fail"))
    normalized.failure_class = "deterministic-policy-violation";
...
function packageIdentity(root, packageRoot) {
  return contentIdentity(root, [
    join(packageRoot, "skill.json"),
    ...walkFiles(join(packageRoot, "src")),
  ]);
}
...
function suiteIdentity(root, packageRoot) {
  return contentIdentity(
    root,
    walkFiles(join(packageRoot, "evals")).filter((path) => !path.includes(`${sep}releases${sep}`)),
  );
}
...
function relativePortable(root, path, label) {
  ...
  if (!isWithin(root, absolute))
    fail(`${label} must remain inside the repository: ${path}`, { code: "unsafe-path" });
...
function materializeFixtures(packageRoot, item) {
  ...
  return (item.files ?? []).map((fixture) => {
    ...
    return {
      source,
      workspace_path:
        target ?? join("inputs", relative(fixtureRoot, sourcePath)).split(sep).join("/"),
...
function prepareDependencies(root, supportPaths) {
  return supportPaths.map((value) => {
    ...
    if (!isWithin(root, source) || source === resolve(root) || !existsSync(source))
      fail(`Unsafe or missing support path: ${value}`, { code: "unsafe-support-path" });
    if (!isWithin(realpathSync(root), realpathSync(source)))
      fail(`Support path resolves outside repository: ${value}`, { code: "unsafe-support-path" });
    if (
      relative(resolve(root), source) !== relative(realpathSync(root), realpathSync(source)) ||
      (statSync(source).isDirectory() && walkSymlinks(source).length)
    )
      fail(`Support path contains or resolves through a symlink: ${value}`, {
        code: "unsafe-support-path",
      });
    return {
      source: relativePortable(root, source, "Support path"),
      workspace_path: relativePortable(root, source, "Support path"),
      content_identity: pathIdentity(source),
    };
  });
}
...
function selectedCases(suite, caseArgs) {
  ...
  if (selectedIds.size !== cases.length)
    fail("One or more selected case ids do not exist", { code: "unknown-case" });
  return cases;
}

function identity(value, status) {
  return { value, status };
}
...
async function preflightRun(args) {
  ...
  for (const [name, value] of Object.entries({
    ...
    if (!value)
      fail(`run requires --${name.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`)}`, {
        code: "missing-argument",
      });
  }
  if (!existsSync(adapter) || !existsSync(graderAdapter))
    fail("Adapter or grader adapter was not found", { code: "missing-adapter" });
  const selectionSource = args.selectionSource ?? "explicit";
  if (!RUNNER_SELECTION_SOURCES.has(selectionSource))
    fail("--selection-source must be explicit or pack-default", {
      code: "invalid-runner-selection",
    });
  for (const [label, path] of [
    ["Target package", packageRoot],
    ["Adapter", adapter],
    ["Grader adapter", graderAdapter],
  ]) {
    if (
      !isWithin(root, path) ||
      path === root ||
      relative(root, path) !== relative(realpathSync(root), realpathSync(path)) ||
      !isWithin(realpathSync(root), realpathSync(path))
    )
      fail(`${label} must resolve directly inside the repository without a symlink`, {
        code: "unsafe-path",
      });
  }
  const findings = [];
  ...
  if (findings.length)
    fail("Suite validation failed", { code: "invalid-suite", details: { findings } });
  if (!EVIDENCE_CLASSES.has(args.evidenceClass ?? "authoring-smoke")) {
    const message =
      args.evidenceClass === "release"
        ? "Protocol 1.0.0 does not support release evidence"
        : `Unsupported evidence class: ${args.evidenceClass}`;
  ...
  const routingCatalogs = new Map(
    cases
      .filter((item) => item.stage === "routing")
      .map((item) => [String(item.id), catalogForCase(root, packageRoot, item)]),
  );
  const evidenceClass = args.evidenceClass ?? "authoring-smoke";
  const trials = Number(
    args.trials ??
      (evidenceClass === "regression"
        ? (contract.trials?.regression_minimum_per_case ?? 3)
        : (contract.trials?.authoring_smoke_minimum ?? 1)),
  );
  const retries = Number(args.retries ?? 0);
  if (!Number.isInteger(trials) || trials < 1)
    fail("--trials must be a positive integer", { code: "invalid-budget" });
  if (!Number.isInteger(retries) || retries < 0)
    fail("--retries must be a non-negative integer", { code: "invalid-budget" });
  ...
  if (
    ![timeoutMs, maximumOutputBytes, maximumInvocations].every(
      (value) => Number.isFinite(value) && value > 0,
    )
  )
    fail("Budgets must be positive numbers", { code: "invalid-budget" });
  if (
    args.tokenBudget !== undefined &&
    (!Number.isInteger(Number(args.tokenBudget)) || Number(args.tokenBudget) < 0)
  )
    fail("--token-budget must be a non-negative integer", { code: "invalid-budget" });
  if (
    args.costBudgetUsd !== undefined &&
    (!Number.isFinite(Number(args.costBudgetUsd)) || Number(args.costBudgetUsd) < 0)
  )
    fail("--cost-budget-usd must be a non-negative number", { code: "invalid-budget" });

  const capabilities = await queryCapabilities(adapter, args, root);
  const graderCapabilities =
    graderAdapter === adapter ? capabilities : await queryCapabilities(graderAdapter, args, root);
  if (
    !graderCapabilities.operations.includes("grade") &&
    cases.some((item) => item.stage === "execution")
  )
    fail("Grader adapter does not support grade", { code: "missing-capability" });
  if (
    cases.some((item) => item.deterministic_assertions?.length) &&
    !capabilities.evidence.includes("tool-calls")
  )
    fail("Deterministic command assertions require structured tool-call evidence", {
      code: "missing-capability",
    });
  for (const stage of new Set(cases.map((item) => item.stage)))
    if (!capabilities.stages.includes(stage))
      fail(`Adapter does not support ${stage} trials`, { code: "missing-capability" });
  if (!capabilities.sandbox_modes.includes(args.sandboxMode))
    fail(`Adapter does not support sandbox ${args.sandboxMode}`, { code: "missing-capability" });
  if (capabilities.network.mode !== args.networkMode)
    fail(
      `Requested network mode ${args.networkMode} is unavailable; adapter provides ${capabilities.network.mode}`,
      { code: "missing-capability" },
    );
  if (
    cases.some((item) => item.stage === "execution") &&
    !graderCapabilities.sandbox_modes.includes(args.sandboxMode)
  )
    fail(`Grader adapter does not support sandbox ${args.sandboxMode}`, {
      code: "missing-capability",
    });
  if (
    cases.some((item) => item.stage === "execution") &&
    graderCapabilities.network.mode !== args.networkMode
  )
    fail(`Grader adapter does not support network mode ${args.networkMode}`, {
      code: "missing-capability",
    });
  if (cases.some((item) => item.stage === "routing")) {
    if (!ROUTING_MODES.has(capabilities.routing_mode))
      fail("Adapter does not support routing", { code: "missing-capability" });
    if (args.routingMode && args.routingMode !== capabilities.routing_mode)
      fail(
        `Requested routing mode ${args.routingMode} is unavailable; adapter provides ${capabilities.routing_mode}`,
        { code: "missing-capability" },
      );
  }
  const requestedBudgets = [
    ["wall-clock", timeoutMs],
    ["output-bytes", maximumOutputBytes],
    ["invocations", maximumInvocations],
    ["tokens", args.tokenBudget],
    ["cost", args.costBudgetUsd],
  ].filter(([, value]) => value !== undefined && value !== null);
  for (const [budget] of requestedBudgets) {
    if (
      !capabilities.enforced_budgets.includes(budget) &&
      !new Set(["wall-clock", "output-bytes", "invocations"]).has(budget)
    )
      fail(`Adapter cannot enforce requested ${budget} budget`, { code: "missing-capability" });
    if (
      cases.some((item) => item.stage === "execution") &&
      new Set(["tokens", "cost"]).has(budget) &&
      !graderCapabilities.enforced_budgets.includes(budget)
    )
      fail(`Grader adapter cannot enforce requested ${budget} budget`, {
        code: "missing-capability",
      });
  }

  const baselineMode = args.baselineMode ?? "none";
  if (!new Set(["none", "without-skill", "package"]).has(baselineMode))
    fail("--baseline-mode must be none, without-skill, or package", { code: "invalid-baseline" });
  if (baselineMode === "package" && !args.baselinePackage)
    fail("--baseline-package is required for package baseline", { code: "invalid-baseline" });
  if (baselineMode !== "package" && args.baselinePackage)
    fail("--baseline-package requires --baseline-mode package", { code: "invalid-baseline" });
  const target = buildTarget(root, packageRoot);
  const configurations = [
    {
      id: safeId(args.configurationId, "configuration id"),
      role: "candidate",
      active_skill: true,
      target,
    },
  ];
  if (baselineMode === "without-skill")
    configurations.push({
      id: safeId(args.baselineConfigurationId ?? "without-skill", "baseline configuration id"),
      role: "baseline",
      active_skill: false,
      target: null,
    });
  if (baselineMode === "package") {
    ...
    if (
      ...
      !isWithin(root, baselineRoot) ||
      baselineRoot === root ||
      relative(root, baselineRoot) !== relative(realpathSync(root), realpathSync(baselineRoot)) ||
      !isWithin(realpathSync(root), realpathSync(baselineRoot))
    )
      baselineFindings.push(
        "Baseline package must resolve directly inside the repository without a symlink",
      );
    validateTargetPackage(baselineRoot, baselineFindings);
    if (baselineFindings.length)
      fail("Baseline package validation failed", {
        code: "invalid-baseline",
        details: { findings: baselineFindings },
      });
    configurations.push({
      id: safeId(args.baselineConfigurationId ?? "baseline", "baseline configuration id"),
      role: "baseline",
      active_skill: true,
      target: buildTarget(root, baselineRoot),
    });
  }
  const planned = plannedInvocationCount(cases, trials, configurations, retries);
  if (planned > maximumInvocations)
    fail(`Planned adapter invocations ${planned} exceed budget ${maximumInvocations}`, {
      code: "invocation-budget-exceeded",
    });
  const manifest = readJson(join(packageRoot, "skill.json"));
  const runId = safeId(
...
function adapterRecord(root, adapter, capabilities) {
  return {
    path: relativePortable(root, adapter, "Adapter"),
    content_identity: pathIdentity(adapter),
    declared_identity: `${capabilities.adapter.name}@${capabilities.adapter.version}`,
    host_cli: capabilities.adapter.host_cli ?? null,
    capabilities,
  };
}
...
function createRun(context, args) {
  ...
  const run = {
    ...
    claim_ceiling:
      evidenceClass === "authoring-smoke"
        ? "Same-author development evidence only"
        : "Bounded reliability for the tested identities",
    target,
    suite: {
      version: suite.suite_version,
      suite_content_identity: suiteIdentity(root, packageRoot),
      contract_version: contract.contract_version,
      contract_content_identity: jsonIdentity(contract),
    },
    runner: {
      implementation: "@agentxm/skills/agent-skill-evaluator",
      version: readJson(join(resolve(skillSourceRoot, ".."), "skill.json")).version,
      content_identity: runnerIdentity(),
      runtime: process.version,
      selection_source: args.selectionSource ?? "explicit",
    },
    adapters: {
      host: adapterRecord(root, adapter, capabilities),
      grader: adapterRecord(root, graderAdapter, graderCapabilities),
    },
    environment: {
      ...
      routing_catalogs: Object.fromEntries(
        [...routingCatalogs].map(([caseId, catalog]) => [
          caseId,
          {
    ...
    authority: {
      policy: identity(args.authorityPolicyId, "declared"),
      network: identity(args.networkMode, capabilities.network.status),
    },
    budgets: {
      ...
      token_budget: {
        value: args.tokenBudget !== undefined ? Number(args.tokenBudget) : null,
        status: args.tokenBudget !== undefined ? "enforced" : "declared",
      },
      cost_budget_usd: {
        value: args.costBudgetUsd !== undefined ? Number(args.costBudgetUsd) : null,
        status: args.costBudgetUsd !== undefined ? "enforced" : "declared",
      },
    ...
    comparison: {
      ...
      ordering: configurations.length > 1 ? "alternating-by-trial" : "single-configuration",
      configurations,
    },
    dependencies,
    coverage: {
      selected_case_ids: cases.map((item) => String(item.id)),
      available_case_ids: suite.evals.map((item) => String(item.id)),
    ...
    provenance: {
      ...
      independence: args.independence ?? "runner and reviewer identities declared by caller",
    },
    invocation: {
  ...
  if (missing.length)
    fail(`Run record does not satisfy required result fields: ${missing.join(", ")}`, {
      code: "unsatisfied-result-contract",
    });
  return run;
}
...
function adapterEnv(context, run) {
  return portableEnvironment(run.invocation.allowed_environment_variables, {
    EVAL_REPO_ROOT: context.root,
    EVAL_MODEL: run.environment.model.value,
    EVAL_TIMEOUT_MS: String(run.budgets.timeout_ms.value),
    EVAL_MAX_OUTPUT_BYTES: String(run.budgets.maximum_output_bytes.value),
    EVAL_SANDBOX_MODE: run.environment.sandbox.value,
    EVAL_NETWORK_MODE: run.authority.network.value,
  });
}
...
async function invokeAdapter({
  ...
  if (run.budgets.consumed.invocations >= run.budgets.maximum_invocations.value)
    return { ok: false, detail: "Invocation budget exhausted" };
  ...
  run.evidence_redaction.private_paths.files_redacted += sanitizeEvidenceTree(outputRoot, [
    env.EVAL_REPO_ROOT,
    process.env.HOME,
  ]);
  if (result.code !== 0 || result.timedOut || result.exceeded)
    return {
      ok: false,
      detail: `adapter exit=${result.code} signal=${result.signal ?? "none"} timeout=${result.timedOut} output_exceeded=${result.exceeded}`,
    };
...
function enforceUsage(run, response) {
  ...
  if (run.budgets.token_budget.value !== null && !Number.isFinite(tokens))
    return "Adapter omitted token usage required for enforcement";
  if (run.budgets.cost_budget_usd.value !== null && !Number.isFinite(cost))
    return "Adapter omitted cost usage required for enforcement";
  if (
    run.budgets.token_budget.value !== null &&
    run.budgets.consumed.tokens > run.budgets.token_budget.value
  )
    return "Token budget exceeded";
  if (
    run.budgets.cost_budget_usd.value !== null &&
    run.budgets.consumed.cost_usd > run.budgets.cost_budget_usd.value
  )
...
function terminalTrial(path) {
  ...
  try {
    const trial = readJson(path);
    return OUTCOMES.has(trial.outcome) ? trial : null;
  } catch {
    return null;
  }
}
...
async function executeRun(context, run) {
    ...
    for (let trialNumber = 1; trialNumber <= context.trials; trialNumber += 1) {
      const configurations =
        item.stage === "execution"
          ? configurationOrder(context.configurations, trialNumber)
      ...
      for (const [configurationIndex, configuration] of configurations.entries()) {
        ...
        const trialRoot = join(
          context.runRoot,
          "trials",
          safeId(String(item.id), "case id"),
          safeId(configuration.id, "configuration id"),
          String(trialNumber),
        );
        ...
        const existingAttemptNumbers = existsSync(attemptsRoot)
          ...
          : [];
        const previousMaximumAttempt = existingAttemptNumbers.length
          ? Math.max(...existingAttemptNumbers)
          : 0;
        for (
          let attemptNumber = previousMaximumAttempt + 1;
          attemptNumber <= previousMaximumAttempt + context.retries + 1;
          attemptNumber += 1
        ) {
          if (cancellationRequested) break;
          ...
          const request = {
            ...
            configuration: {
              ...
              active_skill: configuration.active_skill,
            },
            ...
            catalog:
              item.stage === "routing" ? context.routingCatalogs.get(String(item.id)).trial : [],
          ...
          const trialResult = await invokeAdapter({
            ...
            try {
              ...
              if (budgetFailure) grade = harnessGrade(item, budgetFailure, "budget");
                ...
                const graderRequest = {
                  ...
                  budgets: {
                    token_remaining:
                      run.budgets.token_budget.value === null
                        ? null
                        : Math.max(0, run.budgets.token_budget.value - run.budgets.consumed.tokens),
                    cost_remaining_usd:
                      run.budgets.cost_budget_usd.value === null
                        ? null
                        : Math.max(
                            0,
                            run.budgets.cost_budget_usd.value - run.budgets.consumed.cost_usd,
                          ),
                  },
                };
                const graderResult = await invokeAdapter({
                  adapter: context.graderAdapter,
                  operation: "grade",
                ...
                if (!graderResult.ok) grade = harnessGrade(item, graderResult.detail, "grader");
                  ...
                  grade = graderBudgetFailure
                    ? harnessGrade(item, graderBudgetFailure, "budget")
                    : normalizeGrade(item, graderResult.value, response);
                }
              }
            } catch (error) {
              grade = harnessGrade(item, error.message);
            }
          }
        ...
        writeJsonAtomic(trialPath, {
          ...
          raw_output_or_durable_locator: finalResponse
            ? `attempts/${finalAttempt}/response.json`
            : null,
          observed_selection_or_abstention:
            item.stage === "routing" ? (finalResponse?.selected ?? null) : null,
  ...
  if (cancellationRequested) {
    ...
    run.evidence_redaction.private_paths.files_redacted += sanitizeEvidenceTree(context.runRoot, [
      context.root,
      process.env.HOME,
    ]);
    writeJsonAtomic(join(context.runRoot, "run.json"), run);
...
function trialRecords(runRoot) {
  return walkFiles(join(runRoot, "trials"))
    .filter((path) => basename(path) === "trial.json")
    .map((path) => ({ ...readJson(path), path: relative(runRoot, path).split(sep).join("/") }))
    .sort((left, right) => left.path.localeCompare(right.path));
}

function emptyCounts() {
  return { pass: 0, fail: 0, unknown: 0, "harness-error": 0 };
}
...
function deriveSummary(runRoot, run = readJson(join(runRoot, "run.json")), contract = null) {
  ...
  const countsByConfiguration = Object.fromEntries(
    [...new Set(records.map((item) => item.configuration_id))].map((id) => [
      id,
      countOutcomes(records.filter((item) => item.configuration_id === id)),
    ]),
  );
  const caseRates = [...new Set(candidate.map((item) => String(item.case_id)))]
    .sort()
    .map((caseId) => {
      ...
      return {
        case_id: caseId,
        counts: caseCounts,
        decidable_pass_rate: decided ? caseCounts.pass / decided : null,
        uncertainty: wilsonInterval(caseCounts.pass, decided),
      };
    });
  const routing = candidate.filter((item) => item.stage === "routing");
  const routingCaseRates = [...new Set(routing.map((item) => String(item.case_id)))].map(
    (caseId) => {
      ...
      return {
        ...
        matched,
        match_rate: attempts.length ? matched / attempts.length : null,
        stable: matched === 0 || matched === attempts.length,
      };
    },
  );
  const selectedAll =
    run.coverage.selected_case_ids.length === run.coverage.available_case_ids.length;
  ...
  if (contract.contract_version === CURRENT_CONTRACT_VERSION) {
    for (const record of candidate) {
      const mappings = (contract.analysis?.critical_assertions ?? []).filter(
        (mapping) => String(mapping.case_id) === String(record.case_id),
      );
      if (mappings.length === 0) continue;
      const gradePath = join(dirname(join(runRoot, record.path)), record.assertion_results);
      const grade = readJson(gradePath);
      for (const mapping of mappings) {
        const assertionResult = grade.assertions.find(
          (item) => item.assertion === mapping.assertion,
        );
        if (!assertionResult || assertionResult.result !== "pass")
          criticalFailures.push({
            ...
            evidence:
              assertionResult?.evidence ?? "Mapped critical assertion was absent from the grade.",
    ...
    for (const record of candidate)
      if (criticalIds.has(String(record.case_id)) && record.outcome !== "pass")
        criticalFailures.push({
          gate: null,
          case_id: String(record.case_id),
          configuration_id: record.configuration_id,
          ...
          evidence: "Legacy contract 2.0.0 case-level critical failure.",
          grade: record.assertion_results,
        });
  }
  ...
  for (const [label, record] of [
    ...
    ["Active catalog identifier", run.environment.active_catalog],
    ["Authority policy", run.authority.policy],
  ])
    if (record.status === "declared")
      limitations.push(`${label} was declared by the caller and not independently verified.`);
  if (run.authority.network.value === "unobserved")
    limitations.push("Network activity was not observed or denied by the adapter.");
  if (claimScope === "selected-cases")
    limitations.push(
      "Conclusion is limited to selected cases and stages; it is not a whole-suite conclusion.",
    );
  if (routing.length && run.coverage.trials_per_case === 1)
    limitations.push(
      "Single-attempt routing does not characterize a stochastic selection surface.",
    );
  if (routingCaseRates.some((item) => !item.stable))
    limitations.push("Routing selection varied across repeated trials; use the recorded rates.");
  if (run.environment.routing_mode.value !== "native-routing" && routing.length)
    limitations.push(
      `Routing evidence is ${run.environment.routing_mode.value}, not native host activation evidence.`,
    );
  if (run.environment.credential_isolation.value !== "verified")
    limitations.push("Adapter credential isolation was not independently verified.");
  if (run.comparison.baseline_mode === "none")
    limitations.push(
      "No baseline was executed; the run does not establish comparative skill value.",
    );
  const summary = {
    schema_version: PROTOCOL_VERSION,
    run_id: run.run_id,
  ...
  run.raw_evidence = walkFiles(runRoot)
    ...
    .map((path) => ({
      ...
      content_identity: pathIdentity(path),
    }));
...
async function runEvaluation(args) {
  let context;
  try {
    context = await preflightRun(args);
  } catch (error) {
    if (error instanceof EvalError) {
      ...
      if (args.package && args.runId) {
        try {
          ...
          proposedWorkspace = join(
            resolve(root, args.outputRoot ?? ".work/evals"),
            manifest.owner,
            "skills",
            manifest.name,
            args.runId,
  ...
  try {
    ...
    if (!summary)
      return { run: readJson(join(context.runRoot, "run.json")), summary: null, exitCode: 3 };
    return {
      run: readJson(join(context.runRoot, "run.json")),
      summary,
      exitCode: summary.conclusion === "Supported" ? 0 : 1,
    };
  } catch (error) {
...
function reconstructContext(root, runRoot, run) {
  ...
  const suite = readJson(join(packageRoot, "evals", "evals.json"));
  const contract = readJson(join(packageRoot, "evals", "evaluation-contract.json"));
  const cases = suite.evals.filter((item) =>
    run.coverage.selected_case_ids.includes(String(item.id)),
  );
  const routingCatalogs = new Map(
    Object.entries(run.environment.routing_catalogs ?? {}).map(([caseId, catalog]) => [
      caseId,
      {
        ...catalog,
        trial: catalog.entries.map(({ name, description }) => ({ name, description })),
      },
    ]),
  );
  return {
...
function verifyResumeIdentity(root, run) {
  ...
  const adapter = resolve(root, run.invocation.adapter);
  const graderAdapter = resolve(root, run.invocation.grader_adapter);
  const matches = (compute, expected) => {
    try {
      return compute() === expected;
    } catch {
      return false;
    }
  };
  if (!matches(() => runnerIdentity(), run.runner.content_identity))
    problems.push("runner identity changed");
  if (!matches(() => packageIdentity(root, packageRoot), run.target.package_content_identity))
    problems.push("target identity changed");
  if (!matches(() => suiteIdentity(root, packageRoot), run.suite.suite_content_identity))
    problems.push("suite identity changed");
  if (run.suite.contract_content_identity) {
    const snapshotPath = join(packageRoot, "evals", "evaluation-contract.json");
    if (!matches(() => jsonIdentity(readJson(snapshotPath)), run.suite.contract_content_identity))
      problems.push("contract identity changed");
  }
  if (!matches(() => pathIdentity(adapter), run.adapters.host.content_identity))
    problems.push("host adapter identity changed");
  if (!matches(() => pathIdentity(graderAdapter), run.adapters.grader.content_identity))
    problems.push("grader adapter identity changed");
  for (const configuration of run.comparison.configurations) {
    ...
    if (
      !matches(
        () => packageIdentity(root, baselineRoot),
        configuration.target.package_content_identity,
      )
    )
      problems.push(`comparison target ${configuration.id} identity changed`);
  }
  for (const dependency of run.dependencies) {
    if (!matches(() => pathIdentity(resolve(root, dependency.source)), dependency.content_identity))
      problems.push(`dependency ${dependency.source} identity changed`);
  }
  for (const [caseId, catalog] of Object.entries(run.environment.routing_catalogs ?? {})) {
    for (const entry of catalog.entries)
      if (!matches(() => pathIdentity(resolve(root, entry.source_path)), entry.content_identity))
        problems.push(`routing catalog entry ${entry.name} for case ${caseId} identity changed`);
    if (jsonIdentity(catalog.entries) !== catalog.content_identity)
      problems.push(`routing catalog record for case ${caseId} changed`);
  }
  if (problems.length)
    fail(`Resume refused: ${problems.join(", ")}`, { code: "resume-identity-conflict" });
}
...
async function resumeEvaluation(args) {
  ...
  if (!RUN_STATES.has(run.state) || run.state === "complete")
    fail(`Run state ${run.state} is not resumable`, { code: "invalid-run-state" });
  if (run.suite.contract_content_identity) {
    const snapshotPath = join(runRoot, "contract.json");
    if (
      !existsSync(snapshotPath) ||
      jsonIdentity(readJson(snapshotPath)) !== run.suite.contract_content_identity
    )
      fail("Resume refused: run contract snapshot identity changed", {
        code: "resume-identity-conflict",
      });
  }
  ...
  return {
    run: readJson(path),
    summary,
    exitCode: summary?.conclusion === "Supported" ? 0 : summary ? 1 : 3,
  };
}
...
function validateCommand(args) {
  ...
  if (packages.length === 0)
    findings.push(
      "No workspace-authored Agent Skill packages were discovered; pass --package explicitly.",
    );
  for (const packageRoot of packages) validateSuite(packageRoot, findings);
  return {
    ok: findings.length === 0,
    packages: packages.map((path) => relative(root, path).split(sep).join("/")),
    findings,
  };
}
...
function summarizeCommand(args) {
  ...
  if (run.state === "running")
    fail("Cannot summarize an incomplete running run", { code: "invalid-run-state" });
  const snapshotPath = join(runRoot, "contract.json");
  let contract;
  if (existsSync(snapshotPath)) {
    contract = readJson(snapshotPath);
    if (
      run.suite.contract_content_identity &&
      jsonIdentity(contract) !== run.suite.contract_content_identity
    )
      fail("Run contract snapshot identity changed", { code: "summary-identity-conflict" });
  } else {
    const packageRoot = resolve(root, run.invocation.package);
    if (suiteIdentity(root, packageRoot) !== run.suite.suite_content_identity)
      fail("Legacy run suite identity changed and no contract snapshot is available", {
        code: "summary-identity-conflict",
      });
...
function inspectCommand(args) {
  ...
  const runRoot = resolve(root, args.run);
  const run = readJson(join(runRoot, "run.json"));
  const summary = existsSync(join(runRoot, "summary.json"))
    ? readJson(join(runRoot, "summary.json"))
    : null;
  return {
    run_id: run.run_id,
    state: run.state,
    ...
    claim_scope: summary?.claim_scope ?? null,
    limitations: summary?.limitations ?? [],
    path: relative(root, runRoot).split(sep).join("/"),
  };
}
  ...
  try {
    args =
      process.argv[2] === "--help" || process.argv[2] === "-h"
        ? { help: true, case: [], supportPath: [], allowEnv: [] }
    ...
    if (args.command === "validate") {
      result = validateCommand(args);
      exitCode = result.ok ? 0 : 2;
    } else if (args.command === "run") {
      const outcome = await runEvaluation(args);
      result = { ok: outcome.exitCode === 0, run: outcome.run, summary: outcome.summary };
      exitCode = outcome.exitCode;
    } else if (args.command === "resume") {
      const outcome = await resumeEvaluation(args);
      result = { ok: outcome.exitCode === 0, run: outcome.run, summary: outcome.summary };
      exitCode = outcome.exitCode;
    ...
    if (args.json) process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      ...
      process.stdout.write(
        result.ok
          ? `Validated ${result.packages.length} Agent Skill suite(s).\n`
    ...
    const normalized =
      error instanceof EvalError
        ? error
        : new EvalError(error.message, { code: "unhandled-error", exitCode: 3 });
    const envelope = {
      ok: false,
