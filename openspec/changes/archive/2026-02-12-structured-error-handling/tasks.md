...
## 1. AppError type and rendering

> **Subagent:** Run this entire phase in a single subagent.

- [ ] 1.1 Create `packages/cli/src/app-error/cli-error.ts` with `AppError` TaggedError (code, what, details, howToFix, cause) and convenience factory
- [ ] 1.2 Create `packages/cli/src/app-error/render.ts` with `renderAppError` (formats `✗ {what} ({code})\n  {details}\n  {howToFix}`) and `renderDefect` (bug diagnostic with report suggestion)
- [ ] 1.3 Create `packages/cli/src/app-error/index.ts` barrel exporting public API
- [ ] 1.4 Write tests for `AppError` construction scenarios (all fields, no howToFix, empty details) in `cli-error.test.ts`
- [ ] 1.5 Write tests for `renderAppError` output format (all fields, no howToFix, empty details) and `renderDefect` output in `render.test.ts`
...
- [ ] 3.1 Update `run()` in `packages/cli/src/runtime/index.ts` to use the three-tier catch: `catchTag("PromptCancelled")` → exit 0, `catchTag("AppError")` → `renderAppError` + exit 1, `catchAll` → `renderDefect` + exit 2
- [ ] 3.2 Write handler-level tests verifying the runtime boundary behavior (AppError renders and exits 1, PromptCancelled exits 0, untyped error renders defect and exits 2)
...
## 4. Migrate handlers to AppError
...
- [ ] 4.1 Migrate `install` handler: replace `InstallError` with `mapError` to `AppError`, remove `InstallError` type, update tests
- [ ] 4.2 Migrate `fork` handler: replace `ForkError` with `mapError` to `AppError`, remove `ForkError` type, update tests
- [ ] 4.3 Migrate `publish` handler: replace `PublishError` with `mapError` to `AppError`, remove `PublishError` type, update tests
- [ ] 4.4 Migrate `update` handler: replace `UpdateError` with `mapError` to `AppError`, remove `UpdateError` type, update tests
- [ ] 4.5 Migrate `init` handler: add `mapError` to `AppError` for domain errors, update tests
- [ ] 4.6 Migrate `list` handler: add `mapError` to `AppError` for domain errors, update tests
- [ ] 4.7 Migrate `uninstall` handler: add `mapError` to `AppError` for domain errors, update tests
