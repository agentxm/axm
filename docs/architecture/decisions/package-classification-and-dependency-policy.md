...
The remaining `cli-update` implementation retains its placement until script
replacement, native inspection, and the remaining preview/progress bindings are
separated. The current ownership of preparation, startup checking,
package-managed upgrade execution, settlement contracts, and CLI assessment is described in
