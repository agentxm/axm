...
const ambiguous = (
  ...
  makeAppError({
    ...
    what: `"${input}" matches more than one ${scope} ${extensionTypePluralSentenceLabels[toExtensionTypePlural(resourceType)]}`,
    details: candidates.map((candidate) => `- ${candidate.fqn}`),
