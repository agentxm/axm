/**
 * The publish result contract (`publish-result-v3`): the typed outcome one
 * publish invocation reports, and the rules that classify it.
 *
 * The feature owns the document's shape, its counts, and its execution
 * status; the application owns emission, telemetry, rendering, and exit
 * codes.
 *
...
export const normalizePublishResult = (result: PublishResultInput): PublishResult => {
