...
- **BREAKING**: Introduce a `CliError` wrapper as the single error type at the runtime boundary — handlers convert domain errors to `CliError` carrying structured details (`what`, `details`, `howToFix`, error code). Anything else reaching the boundary is treated as a defect.
- Add a structured error renderer at the runtime boundary that formats `CliError` consistently, with verbose cause chain output for debugging
- **BREAKING**: Remove `formatError(what, details, howToFix)` string builder — replaced by structured fields on `CliError`
...
- `cli-error`: `CliError` wrapper type, error renderer, error codes, structured details — the complete error boundary contract

### Modified Capabilities

- `cli`: Runtime boundary catches `CliError | PromptCancelled` only; unhandled errors are defects
- `tui-spinner`: Ensure spinner stops cleanly on all error paths before error rendering

## Impact

- `packages/cli/src/runtime/` — `CliError` type, error renderer, `PromptCancelled` handling
- `packages/cli/src/utils/errors.ts` — remove `formatError`, replace with `CliError` constructor
- All handlers — add final `mapError` converting domain errors to `CliError`
