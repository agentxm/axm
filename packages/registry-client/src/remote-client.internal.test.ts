...
/**
 * Helper to run an effect and extract the typed registry failure.
 */
const runFailure = <A>(effect: Effect.Effect<A, RegistryClientFailure>) => Effect.flip(effect);
...
describe("previewExtensionPublishes", () => {
  ...
  it.effect.each([
    ...
    Effect.gen(function* () {
      ...
      expect(error.category).toBe(expectedCode);
