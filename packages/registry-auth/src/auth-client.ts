...
export class AuthClient extends ServiceMap.Service<AuthClient, AuthClientService>()(
  "@agentxm/registry-auth/auth-client/AuthClient",
) {}
...
const mapRegistryAuthError = (
  registryUrl: string,
  operation: string,
  error: unknown,
): RegistryClientFailure => registryAuthFailure(registryUrl, operation, error);
...
const retryTransientDevicePollFailure = <A>(
  ...
  effect.pipe(
    ...
    Effect.catchTag("RetryableDevicePollError", (e) =>
      Effect.fail(registryAuthFailure(registryUrl, "Device token exchange failed", e.cause)),
    ),
  );
