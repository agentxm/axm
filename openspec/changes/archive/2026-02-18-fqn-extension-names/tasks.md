...
- [x] 1.2 Create `packages/cli/src/extensions/fqn.ts` with: `ExtensionTypePlural` type (`"skills" | "packs" | "mcp-servers"`), `Fqn` type (`{ readonly namespace: string; readonly type: ExtensionTypePlural; readonly name: string }`), `parseFqn(input: string): Effect<Fqn, AppError>`, `parseFqnOrThrow(input: string): Fqn`, `formatFqn(fqn: Fqn): string`
