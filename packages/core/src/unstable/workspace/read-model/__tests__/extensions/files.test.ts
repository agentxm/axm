 ...
 * exclusively from the canonical-extensions scanner (`type === "files"`); no
 * agent registers a files rendering directory.
 */
...
import { makeFilesExtensionsApi } from "../../extensions/files.js";
...
const harness = (params: {
  ...
  Effect.gen(function* () {
    ...
    return yield* makeFilesExtensionsApi({
      scope: "project",
      ...
      const api = yield* harness({
        canonicalOccurrences: [
          makeCanonicalOccurrence({
            ...
            name: "readme",
            owner: null,
            contentLocation: "/ws/.axm/extensions/extern/files/readme",
          }),
        ],
      });
