...
import { InvalidAgentId, LockedSkillMissing, SettingsEntryMissing } from "../index.js";
import type { SourceHostConfig } from "../index.js";
import type { McpServerLockEntry, SkillLockEntry, SubagentLockEntry } from "../index.js";
import {
  exactVersion,
  expectDefined,
  extensionName,
...
import { computeSourceHash } from "../index.js";
import { TreeIntegritySchema } from "../index.js";
import { computePackManifestContentIdentity } from "../index.js";
import { mcpResolutionKey, mcpRegistryResolutionKey } from "../index.js";
import { PackManifestSchema } from "@agentxm/extension-model/unstable/packs/manifest-schema";
import { layer as workspaceLayer } from "../live.js";
import {
  WorkspaceMutations,
...
import { configuredRowsByName, installedRowsByName, unmanagedRowsByName } from "../index.js";
      ...
      for (const failure of failures) {
        it.effect(`blocks construction for ${source.owner} ${failure.name}`, () =>
          Effect.gen(function* () {
            ...
            expect(error).toMatchObject({ _tag: failure.tag, path: settingsPath });
  ...
  describe("getConfiguredOwner", () => {
    ...
    it.effect("rejects bare owner in settings", () =>
      Effect.gen(function* () {
        ...
        const error = yield* getService(defaultOptions).pipe(Effect.flip);
        expect(error).toMatchObject({ _tag: "SettingsDecodeError" });
      }),
    );
  });
