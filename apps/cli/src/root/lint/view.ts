...
import { count, scopePhrase } from "../../screen/index.js";
...
const exitPart = (exitCode: number): string | undefined =>
  exitCode === 0 ? undefined : `exit ${String(exitCode)}`;
...
export const lintDoc = (input: LintViewInput): Doc => {
  ...
  return [
    ...
    { _tag: "blank" },
    ...drift,
    ...(drift.length === 0 ? [] : [{ _tag: "blank" } as const]),
    ...
    ...(fixable === 0 || input.fix
      ? []
      : [{ _tag: "blank" } as const, { _tag: "next", actions: [fixSuggestion(fixable)] } as const]),
  ];
};
