...
/**
 * Render the ownership banner for every comment-bearing managed-file format,
 * so a native renderer can place its own without knowing the wording.
 */
export const managedFileBanners = (
  options: Omit<ManagedFileBannerOptions, "format">,
): { readonly markdown: string; readonly toml: string } => ({
  markdown: makeMarkdownBanner({ ...options, format: "markdown" }),
  toml: makeTomlBanner({ ...options, format: "toml" }),
});
