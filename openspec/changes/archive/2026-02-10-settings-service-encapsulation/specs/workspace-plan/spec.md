...
- **WHEN** project settings does not contain a `scope` field but global settings contains `namespace: "@corp"`
...
- **THEN** `workspace.getConfiguredNamespace()` returns the default scope `"@community"`
