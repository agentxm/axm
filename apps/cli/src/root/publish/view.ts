...
const tally = (placed: ReadonlyArray<Placed>, exitCode: number): string => {
  ...
  if (result.mode === "preview") {
    if (failed.length > 0) {
      return {
        ...
        aside: joined([problemAside, "nothing was uploaded"]),
      };
    }
...
export const publishDoc = (result: PublishResult, options: PublishDocOptions): Doc => {
  ...
  return [
    ...
    ...(preview && !detailed && ledger.length > 0 ? [dim(DETAIL_HINT)] : []),
    ...next,
  ];
};

/**
 * Render the human publish result. Quiet keeps the terminal silent, as it
 * always has for publish, so a failure still reaches a person through the
 * problem the invocation ends with.
 *
...
export const renderHumanPublishResult = (
  ...
  Effect.gen(function* () {
    ...
    if (verbosity.level === "quiet") return false;
