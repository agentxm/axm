...
import { handleUninstallContext } from "./handler.js";
...
export const uninstallCommand = Command.make(
  ...
  ({ name, scope, yes, force, preview }) =>
    handleUninstallContext({ name }, { yes, force, preview }).pipe(
      withWorkspace(scope),
      withRuntime("context uninstall"),
    ),
).pipe(
  withArgvTracking(uninstallConfig),
  Command.withDescription("Uninstall a context package"),
  Command.withExamples([
    {
      command: "axm context uninstall workspace-baseline",
      description: "Uninstall a context package by name",
    },
  ]),
);
