...
} from "../../composition/native/index.js";
import { Homebrew, type InstallMethodType } from "../../domain/index.js";
...
import { toUpgradeAssessment, type UpgradeAssessmentResult } from "../cli/index.js";

export { InstallMethodTest } from "./install-method/install-method.js";
export { makeUpdateCheckCacheLayer } from "../../composition/native/update-cache.js";
 ...
 * without delivery services. Recorders remain readable after settlement or
 * failure, so a refusal can be observed to have touched nothing.
 */
export const makeUpgradeTrial = (
  options?: UpgradeTrialOptions,
): Effect.Effect<UpgradeTrialFixture> =>
  Effect.sync(() => {
    const method =
      options?.method ??
      new Homebrew({
...
/**
 * Run one upgrade against a stand-in installation and release origin and
 * return its assessment and native effects.
 */
