/**
 * files package materialization helpers.
 *
...
export interface FileTemplateFiles {
...
const resolvePayloadPath = (packageRoot: string, payloadPath: string) =>
  Effect.gen(function* () {
    ...
    if (isEscapingRelative(path, srcRoot, resolved)) {
      return yield* failValidation(`files package payload path escapes src/: ${payloadPath}`);
    }
    return resolved;
  });
...
const resolveWorkspaceTarget = (workspaceRoot: string, target: string) =>
  Effect.gen(function* () {
    ...
    if (Option.isNone(relative)) {
      return yield* failValidation(`files package target escapes workspace: ${target}`);
    }
    return {
  ...
  Effect.gen(function* () {
    ...
    return yield* fs.readFileString(resolved).pipe(
      Effect.mapError((error) =>
        makeAppError({
          code: "internal",
          detail: `Failed to read files payload: ${payloadPath}`,
          cause: error,
        }),
      ),
    );
...
const renderGeneratedContent = (
  generator: FileGeneratorSpec,
  files: FileTemplateFiles,
  generatedFiles?: RenderFileContentArgs["generatedFiles"],
    ...
    switch (generator.name) {
      ...
      case "toc": {
        ...
        const generatedOwnRegion =
          generatedFiles?.ownRegion !== undefined && Option.isSome(style)
            ? { marker: generatedFiles.ownRegion, style: style.value }
            : undefined;
        const ownRegion =
...
const resolveTemplateExpression = (
  ...
  switch (namespace) {
    ...
    case "workspace": {
      ...
      return Effect.succeed(files.workspace.root);
    }
    default:
...
export const renderFileTemplate = (
  ...
  Effect.gen(function* () {
    ...
    while (match !== null) {
      ...
      output += yield* resolveTemplateExpression(expression, files);
...
export const renderFileContent = ({
  ...
  Effect.gen(function* () {
    switch (source.kind) {
        ...
        const chunks = yield* Effect.forEach(
          ...
          (payloadPath) =>
            readPayload(packageRoot, payloadPath).pipe(
              Effect.flatMap((content) => renderFileTemplate(content, templateFiles)),
            ),
          { concurrency: 1 },
        );
        return chunks.join("");
      ...
      case "generated":
        return yield* renderGeneratedContent(source.generator, templateFiles, generatedFiles);
    }
  });
  ...
  Effect.gen(function* () {
    ...
    yield* fs.writeFileString(absoluteTarget, content).pipe(
      Effect.mapError((error) =>
        makeAppError({
          code: "internal",
          detail: `Failed to write files target: ${absoluteTarget}`,
          cause: error,
        }),
      ),
    );
...
> =>
  Effect.gen(function* () {
    ...
    const content = yield* renderFileContent({
      ...
      source: entry.source,
      templateFiles,
      generatedFiles: { target: entry.target },
    });
