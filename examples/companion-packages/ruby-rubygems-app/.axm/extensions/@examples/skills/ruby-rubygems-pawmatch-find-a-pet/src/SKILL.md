---
name: ruby-rubygems-pawmatch-find-a-pet
description: Help an end user identify and apply for an adoptable pet using the PawMatch Ruby CLI.
---

# Find a Pet with PawMatch

Use this skill when a user wants help finding a pet to adopt and the
`pawmatch` Ruby CLI is available (installed as a gem or run via
`bundle exec pawmatch`). The skill drives `pawmatch` to surface candidates,
