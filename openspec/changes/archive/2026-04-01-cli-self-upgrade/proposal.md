...
- Add a passive update-available notification that checks the latest release version periodically (at most once per 24 hours) and prints a short notice when a newer version exists
...
- `cli-update-check`: Periodic background version check and update notification displayed before command output
