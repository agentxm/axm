---
name: zig-zon-pawmatch-find-a-pet
...
`pawmatch` Zig CLI is available (either as a built binary at
`./zig-out/bin/agentxm-example-pawmatch` or via `zig build run --` from this
repository). The skill drives `pawmatch` to surface candidates, explain
adoption costs, and start an application. It never modifies the codebase.
