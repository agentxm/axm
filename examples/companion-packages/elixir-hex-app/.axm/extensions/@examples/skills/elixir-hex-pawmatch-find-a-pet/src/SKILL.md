---
name: elixir-hex-pawmatch-find-a-pet
...
`pawmatch` Elixir CLI is available (either as a built escript on `$PATH`, as
`./pawmatch` from `mix escript.build`, or via `mix run --no-start -e
'AgentXM.Examples.PawMatch.CLI.run([...])'` from this repository). The skill
drives `pawmatch` to surface candidates, explain adoption costs, and start an
application. It never modifies the codebase.
