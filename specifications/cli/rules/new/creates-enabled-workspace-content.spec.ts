...
export const specification = defineSpecification({
  ...
  statement:
    "When a person creates a rule, AXM shall create its type-specific manifest and starter content in the workspace authoring directory and register it as enabled workspace-authored content with the supplied authoring options.",
  class: "functional",
  role: "experience",
  goals: ["authoring-and-creation", "workspace-intent-fidelity"],
  methods: ["example"],
  derivedFrom: ["packages/cli/src/root/rules/new.ts"],
  supersedes: [],
  assumptions: [],
  openQuestions: [],
});
...
describe("Creating a rule", () => {
  ...
  it.effect("creates editable content and an enabled workspace declaration", () =>
    Effect.gen(function* () {
      ...
      const row = authoringTypes.find((item) => item.type === "rule");
      ...
      expect(readPackageJson(created.root, "rules/review/rule.json")).toMatchObject({
        owner: "@acme",
        type: "rule",
        name: "review",
        ...{ title: "Review policy" },
      ...
      expect(created.readFile("rules/review/src/RULE.md")).toContain("Review policy");
      expect(created.readFile("AGENTS.md")).toContain("Review policy");
    }),
  );
});
