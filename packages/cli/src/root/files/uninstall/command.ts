...
import { handleUninstallFiles } from "./handler.js";
...
export const uninstallCommand = Command.make(
  ...
  ({ name, scope, yes, force, preview, keepSource, deleteSource }) =>
    Effect.gen(function* () {
      ...
      yield* handleUninstallFiles(
        { name },
        { yes, force, preview, ...(sourceDisposition === undefined ? {} : { sourceDisposition }) },
      );
    }).pipe(withWorkspace(scope), withRuntime("files uninstall")),
).pipe(
  withArgvTracking(uninstallConfig),
  Command.withDescription("Uninstall a files package"),
  Command.withExamples([
    {
      command: "axm files uninstall workspace-baseline",
      description: "Uninstall a files package by name",
    },
  ]),
);
