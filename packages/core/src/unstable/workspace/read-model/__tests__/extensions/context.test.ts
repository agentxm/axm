/**
 * context subject module tests.
 *
 * context have no settings entry shape and no lockfile entry shape in v1, so
 * declared and resolved are always `Option.none()`. Actual occurrences come
 * exclusively from the canonical-extensions scanner (`type === "context"`); no
 * agent registers a context rendering directory.
 */
...
import { makeContextExtensionsApi } from "../../extensions/context.js";
...
const harness = (params: {
  ...
  Effect.gen(function* () {
    ...
    return yield* makeContextExtensionsApi({
      scope: "project",
...
describe("makeContextExtensionsApi", () => {
      ...
      const api = yield* harness({
        canonicalOccurrences: [
          makeCanonicalOccurrence({
            ...
            name: "license",
            owner: "@owner",
            contentLocation: "/ws/.axm/extensions/@owner/context/src/license",
          }),
        ],
      });
      ...
      const api = yield* harness({
        canonicalOccurrences: [
          makeCanonicalOccurrence({
            ...
            name: "readme",
            owner: null,
            contentLocation: "/ws/.axm/extensions/external/context/readme",
          }),
        ],
      });
