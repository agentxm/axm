...
export const updateCommand = Command.make(
  ...
  ({ scope, yes, force, preview }) =>
    handleWorkspaceUpdate({
      command: "context-files.update",
      type: Option.some("file"),
      planName: "Update files",
      planDescription: Option.some("Update configured context files packages"),
      flags: { yes, force, preview },
    }).pipe(withWorkspace(scope), withRuntime("context-files update")),
).pipe(
  withArgvTracking(updateConfig),
  Command.withDescription("Update configured context files packages"),
  Command.withExamples([
    {
      command: "axm context-files update",
      description: "Update configured context files packages",
    },
  ]),
);
