...
const decodeManifest = Schema.decodeUnknownEffect(ContextFilesManifestSchema);
...
const toAppError = (error: AppError | unknown): AppError =>
  isAppError(error)
    ...
    : makeAppError({
        code: "internal",
        detail: "Failed to publish context files package",
        cause: error,
      });

const publishFile = (args: {
...
> =>
  Effect.gen(function* () {
    ...
    const manifestPath = path.join(extensionDir, CONTEXT_FILES_MANIFEST_FILENAME);
      ...
      Effect.flatMap((content) =>
        Effect.try({
          try: (): unknown => JSON.parse(content),
            ...
            makeAppError({
              code: "validation",
              detail: `Invalid JSON in ${CONTEXT_FILES_MANIFEST_FILENAME}`,
              cause: error,
            }),
        }),
      ),
    );
    const manifest: ContextFilesManifest = yield* decodeManifest(manifestJson).pipe(
      Effect.mapError((error) =>
        makeAppError({
          code: "validation",
          detail: `Invalid ${CONTEXT_FILES_MANIFEST_FILENAME}`,
          cause: error,
        }),
      ),
    );
...
export const handleContextFilesPublish = Effect.fn("ContextFilesPublish.handle")(function* (args: {
  readonly input: string;
  readonly registry: Option.Option<string>;
  ...
  const runPublishPlan = Effect.gen(function* () {
          ...
          steps: [
            {
              readiness: "ready",
              label: args.input,
              run: publishFile({ input: args.input, registry: args.registry }).pipe(
    ...
    yield* emitPlanResolutionResult("context-files.publish", resolution);
...
const publishConfig = {
  input: Argument.string("input").pipe(
    Argument.withDescription("Context files package FQN (@owner/files/name)"),
  ),
...
export const publishCommand = Command.make(
  ...
  ({ input, registry, scope, yes, force, preview }) => {
    const program = handleContextFilesPublish({ input, registry, yes, force, preview }).pipe(
      withWorkspace(scope),
    );
    return program.pipe(Effect.provide(AuthLayer), withRuntime("context-files publish"));
  },
).pipe(
  withArgvTracking(publishConfig),
  Command.withDescription("Publish a context files package"),
  Command.withExamples([
    {
      command: "axm context-files publish @acme/files/workspace-baseline",
      description: "Publish a context files package",
    },
  ]),
);
