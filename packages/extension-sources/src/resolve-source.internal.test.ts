...
import { SourceHostNotConfigured, SourceNotResolvable } from "./errors.js";
...
const makeWorkspaceLayer = (
  sources: ReadonlyArray<ConfiguredSourceHost>,
  _skills: Record<string, never> = {},
  registrySources?: ReadonlyArray<RegistrySourceHost>,
) =>
  Layer.merge(
    Layer.merge(
      makeTestWorkspaceCatalog({
        sources,
        ...(registrySources === undefined ? {} : { registrySources }),
      }),
      remoteHttpLayer,
    ),
    NodeServices.layer,
...
const BUILT_IN_SOURCES: ReadonlyArray<ConfiguredSourceHost> = [
...
describe("resolveSource", () => {
  ...
  describe("typed name resolution", () => {
    it.effect("reports an unknown MCP server with MCP-specific recovery", () =>
      Effect.gen(function* () {
        ...
        expect(error).toBeInstanceOf(SourceNotResolvable);
        if (!(error instanceof SourceNotResolvable))
          throw new Error("Expected SourceNotResolvable");
  ...
  describe("registry resolution", () => {
    it.effect("resolves @owner/skills/name with registry host config", () =>
      Effect.gen(function* () {
        const registryConfig: RegistrySourceHost = {
          name: "agentxm",
          type: "registry",
          location: new URL("https://registry.example.com"),
        };
        const sources: ReadonlyArray<ConfiguredSourceHost> = [
          registryConfig,
    ...
    it.effect("resolves registry source", () =>
      Effect.gen(function* () {
        ...
        const sources: ReadonlyArray<ConfiguredSourceHost> = [...BUILT_IN_SOURCES, registryConfig];
  ...
  describe("no config for source type", () => {
    it.effect("fails when no config exists for source type", () =>
      Effect.gen(function* () {
        ...
        expect(error).toBeInstanceOf(SourceHostNotConfigured);
        expect(error.detail).toContain('source named "github"');
      }),
    );
  });
  ...
  describe("multi-config URL hostname matching", () => {
    const multiGitHubSources: ReadonlyArray<ConfiguredSourceHost> = [
      { name: "github", type: "github", url: new URL("https://github.com") },
