...
Each rendered command file SHALL start with a static managed-by header appropriate to its format. The header identifies AXM ownership and points to the relevant CLI help for discoverability.
...
- **THEN** the first line of the rendered file SHALL be `<!-- Managed by axm — see "axm commands --help" -->`
...
#### Scenario: Sync always overwrites managed files

- **WHEN** a managed command file has been manually edited
- **AND** `axm sync` is run
- **THEN** the file SHALL be overwritten with the rendered content from the frontmatter and command body

### Requirement: Install conflict detection

When rendering a command file, the adapter SHALL detect conflicts with pre-existing files. The `--force` flag SHALL override conflict blocking.
...
#### Scenario: Existing file with managed header

- **WHEN** the render target path has an existing file with the axm managed-by header
- **THEN** the adapter SHALL re-render and overwrite the file

#### Scenario: Existing file without managed header blocks install

- **WHEN** the render target path has an existing file without the axm managed-by header
- **AND** `--force` is not passed
- **THEN** the adapter SHALL block rendering with a conflict error indicating a pre-existing manual file

#### Scenario: Force overrides conflict

- **WHEN** the render target path has an existing file without the axm managed-by header
- **AND** `--force` is passed
- **THEN** the adapter SHALL render and overwrite the file with the managed header
