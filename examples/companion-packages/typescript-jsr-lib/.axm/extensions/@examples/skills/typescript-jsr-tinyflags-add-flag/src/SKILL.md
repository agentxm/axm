---
name: typescript-jsr-tinyflags-add-flag
description: Add a TinyFlags flag to a JSR TypeScript Deno project with implementation, tests, and rollout notes.
---

# Add TinyFlags Flag

Use this skill when adding a feature flag to a Deno/JSR TypeScript project
that uses `@agentxm/example-tinyflags`.

## Workflow

1. Find the module that constructs the TinyFlags client with `tinyFlags`.
2. Add the flag with `booleanFlag` or `variantFlag`.
3. Prefer camelCase or kebab-case flag keys consistent with the existing
   project style.
4. Add or update `Deno.test` coverage for default behavior and rollout
   behavior. Use `jsr:@std/assert` for assertions.
