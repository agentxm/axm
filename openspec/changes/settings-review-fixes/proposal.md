...
- Validate `agentId` in `SettingsService.addAgent` using `AgentIdSchema` instead of casting with `as AgentId`
...
- `settings-service`: `addAgent` now validates the agent ID against `AgentIdSchema` and fails with a typed error for invalid IDs (previously accepted any string and cast internally)
...
- **Test files**: Update `as Settings` casts and adjust for required `cause` on error types
