/**
 * Tests for the `WorkspaceContext` `Context.Service` and the
 * `WorkspaceContextLive` Layer (Phase 9).
 *
 * Covers, against the fixture builder:
 *
 * (a) `WorkspaceContext` is a single `Context.Service` tagged
 ...
 * (e) The cached effect count per `WorkspaceContext` instance stays within
 *     ≤50 on a representative full-workspace fixture. Verified via a debug
 *     hook on `WorkspaceContextLive`.
...
describe("WorkspaceContext service tag", () => {
  it("is a single Context.Service tagged `axm/WorkspaceContext`", () => {
    expect(WorkspaceContext.key).toBe("axm/WorkspaceContext");
    expect(Context.isKey(WorkspaceContext)).toBe(true);
  });
});
...
describe("scoped cells are dependency-closed at the call site", () => {
  it.effect("project.skills.declared can be yielded with WorkspaceContext only", () =>
    Effect.gen(function* () {
      const layer = WorkspaceContextTest(validAll(WORKSPACE_ROOT, USER_HOME));
      yield* Effect.gen(function* () {
        const ctx = yield* WorkspaceContext;
...
describe("cached effect budget", () => {
  it.effect("≤50 cached effects per WorkspaceContext instance on a representative fixture", () =>
    Effect.gen(function* () {
      const layer = WorkspaceContextTest(validAll(WORKSPACE_ROOT, USER_HOME));
      yield* Effect.gen(function* () {
...
// ---------------------------------------------------------------------------
// (f) Workspace-root escape fails the Layer
// ---------------------------------------------------------------------------
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const escapingLayer = WorkspaceContextTest(validAll(WORKSPACE_ROOT, USER_HOME), {
