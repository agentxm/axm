...
export const CONTEXT_FILES_JSON = "context-files.json";
export const SRC_ROOT = "src";

const decoder = new TextDecoder();

export const decodeContextFilesManifest = (input: unknown): Option.Option<ContextFilesManifest> => {
  const result = Schema.decodeUnknownResult(ContextFilesManifestSchema)(input, {
...
export const advisory = (
  ...
  severity: Severity,
  message: string,
  file: string = CONTEXT_FILES_JSON,
): AdvisoryFinding => ({
  kind: "advisory",
  ruleId,
  severity,
...
export const readPayloadString = (
  context: ContextFilesRuleContext,
  payloadPath: string,
