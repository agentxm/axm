...
`../golang-lib/.axm/extensions/` are designed to operate on.

`pawmatch` is not publishable — it exists to demonstrate consumption, not to
publish.

The app also ships its own companion AXM skill,
[`golang-pawmatch-find-a-pet`](./.axm/extensions/@examples/skills/golang-pawmatch-find-a-pet/src/SKILL.md),
...
replace github.com/agentxm/example-tinyflags => ../golang-lib
```
