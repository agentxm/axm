...
import {
  ...
  type TestWorkspaceCatalogOptions,
...
const provide = (
  ...
  options?: {
    readonly nodes?: TestWorkspaceCatalogOptions["desiredExtensionGraph"];
  ...
  effect.pipe(
    Effect.provide(
      Layer.mergeAll(
        makeTestWorkspaceCatalog({
          ...(options?.nodes === undefined ? {} : { desiredExtensionGraph: options.nodes }),
          registrySources: [
            {
              name: "agentxm",
              type: "registry",
...
describe("resolveIdentifier", () => {
  ...
  it.effect("ignores stale receipt identities", () =>
    Effect.gen(function* () {
      const resolved = yield* provide(
        ...
        // Lock receipts are not part of the catalog port surface; resolution
  ...
  it.effect("fails when a bare name is not found", () =>
    Effect.gen(function* () {
      ...
      expect(result._tag).toBe("Failure");
      if (result._tag === "Failure") {
        expect(sourceResolutionFailureCategory(result.failure)).toBe("not_found");
      }
    }),
  );
});
...
/**
 * One desired-graph node per catalog type. The `satisfies Record<…>` keeps
...
const desiredGraphByType = {
  skill: desiredGraphFor("skill"),
  "mcp-server": desiredGraphFor("mcp-server"),
  subagent: desiredGraphFor("subagent"),
  rule: desiredGraphFor("rule"),
  hook: desiredGraphFor("hook"),
  knowledge: desiredGraphFor("knowledge"),
} satisfies Record<CatalogExtensionType, TestWorkspaceCatalogOptions["desiredExtensionGraph"]>;
