...
import {
  ...
  TARGET_VERSION,
  runUpgradeCommand,
} from "../../test-support/upgrade-harness.js";
...
describe("Availability diagnosis", () => {
  it.effect.each(cases)("attributes $command $response.executionState as $state", (scenario) =>
    Effect.gen(function* () {
      ...
      const machine = yield* runUpgradeCommand(options);
      const human = yield* runUpgradeCommand({ ...options, human: true });
  ...
  it.effect.each([
    ...
    Effect.gen(function* () {
      const machine = yield* runUpgradeCommand({ formulaVersion: scenario.version });
      const human = yield* runUpgradeCommand({ formulaVersion: scenario.version, human: true });
      expect(machine.document).toMatchObject({
        result: {
          disposition: `installer-${scenario.state}`,
          details: {
