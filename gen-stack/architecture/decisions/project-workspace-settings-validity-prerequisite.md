...
title: Project workspace settings validity prerequisite
description: The accepted decision that project workspace construction fails
  before operation execution when either present settings source is invalid.
status: stable
tags: [workspace, settings, validation, fail-fast, decision]
sources:
  ...
  - id: workspace-invariants
    resource: https://github.com/agentxm/axm/blob/4db7b4ce74eec1b9fc839ea8da3e4e3e027e2ee4/docs/architecture/workspace/invariants.md
    title: AXM workspace invariants architecture
...
values and policies from user settings. Workspace construction currently reads
and validates both sources before resolving layouts or providing workspace
...
workflow. The accompanying Requirement owns the binding outcome; this record
owns the architectural response and tradeoff.
...
must explicitly reconcile the Requirement, CLI Surface, Evaluation Protocols,
and detailed workspace architecture.
