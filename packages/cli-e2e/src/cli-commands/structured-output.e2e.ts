 ...
 * - stream-json: log messages emit as NDJSON events
...
const getJsonLines = (result: { stdout: string; stderr: string }): ReadonlyArray<string> =>
  `${result.stdout}\n${result.stderr}`
    .trim()
    .split("\n")
...
describe("structured output modes", () => {
  ...
  describe("--output-format stream-json", () => {
    it("emits CliRenderer messages as NDJSON log events", async () => {
      const temp = createTempDir();
      try {
        ...
        const lines = getJsonLines(result);
  ...
  describe("error routing", () => {
    it("routes runtime errors as JSON on stdout in json mode (exit 1)", async () => {
      ...
      try {
        ...
        expect(errorJson.code).toBe("AUTH_LOGIN_REQUIRED");
    ...
    it("routes runtime errors as NDJSON in stream-json mode", async () => {
      const temp = createTempDir();
      try {
        const result = await runCli(["token", "--output-format", "stream-json"], {
          cwd: temp.path,
          env: { AXM_TOKEN: "" },
        });

        expect(result.exitCode).toBe(1);
        const jsonLine = expectDefined(getJsonLines(result)[0]);
        const errorJson = JSON.parse(jsonLine);
        expect(errorJson.type).toBe("error");
        expect(errorJson.code).toBe("AUTH_LOGIN_REQUIRED");
        expect(result.stderr).toContain("No token available");
