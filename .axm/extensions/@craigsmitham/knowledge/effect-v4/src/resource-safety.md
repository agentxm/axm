...
sources:
  - id: docs-acquire-release
    ...
    title: Official Effect docs — acquireRelease inside Layer.effect for a scoped service (effect 4.0.0-rc.110)
  - id: src-effect
    resource: https://github.com/Effect-TS/effect/blob/effect%404.0.0-rc.110/packages/effect/src/Effect.ts
    title: Effect module source — acquireRelease, acquireUseRelease, acquireDisposable, addFinalizer, ensuring, interruption masks (effect 4.0.0-rc.110)
  - id: src-scope
    resource: https://github.com/Effect-TS/effect/blob/effect%404.0.0-rc.110/packages/effect/src/Scope.ts
    title: Scope module source — make, provide, fork, close, finalizer strategy (effect 4.0.0-rc.110)
  - id: test-scope
    resource: https://github.com/Effect-TS/effect/blob/effect%404.0.0-rc.110/packages/effect/test/Scope.test.ts
    title: Scope tests — finalizer ordering and parallel finalization (effect 4.0.0-rc.110)
...
- For a shared application service, acquire with `Effect.acquireRelease`
  inside `Layer.effect`, whose scope owns the finalizer — rc.110 has no
...
- Register release immediately after acquisition; do not leave an
  interruptible gap. In rc.110 the acquisition step of `acquireRelease` and
