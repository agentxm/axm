---
name: perl-cpan-pawmatch-find-a-pet
description: Help an end user identify and apply for an adoptable pet using the PawMatch Perl CLI.
---

# Find a Pet with PawMatch

Use this skill when a user wants help finding a pet to adopt and the
`pawmatch` Perl CLI is available (installed as a CPAN distribution or run
via `perl -Ilib bin/pawmatch`). The skill drives `pawmatch` to surface
