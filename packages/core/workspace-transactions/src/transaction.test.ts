...
/**
 * Run one transaction in a fresh scope for `workspaceDir`. Tests declare
 * their targets explicitly, so the scope's default settings and lock targets
 * are not claimed.
 */
const runWorkspaceTransaction = <A, E, R>(
  args: WorkspaceTransactionArgs<A, E, R> & { readonly workspaceDir: string },
) => {
  const { workspaceDir, ...rest } = args;
  return runWorkspaceTransactionInScope({ claimDefaultTargets: false, ...rest }).pipe(
    Effect.provide(scopeFor(workspaceDir)),
  );
};

const detailOf = (
  error: StepFailure | WorkspaceTransactionFailure | WorkspaceRestorationIncomplete,
): string =>
  error._tag === "StepFailure"
    ? error.detail
            ...
            transition: Effect.sync(() => nodeFs.writeFileSync(laterTarget, "created\n")).pipe(
              Effect.andThen(
                Effect.fail(
                  new StepFailure({
                    category: "internal",
                    detail: "injected later-target failure",
                  }),
                ),
              ),
            ),
      ...
      runWorkspaceTransaction({
        ...
        Effect.tap((error) =>
          Effect.sync(() => {
            ...
            expect(
              Option.getOrUndefined(Cause.findErrorOption(error.transitionCause)),
            ).toMatchObject({ detail: "injected transition failure" });
    ...
    return withContext(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const failure = yield* runWorkspaceTransactionInScope({
  ...
  it.effect("serializes concurrent transitions for one workspace", () =>
    withContext(
      Effect.gen(function* () {
        ...
        // Both transitions share one scope: its admission permit serializes
        // them before either reaches the process lock.
        const transition = runWorkspaceTransactionInScope({
          claimDefaultTargets: false,
          targets: [settingsPath],
          transition: Effect.gen(function* () {
      ...
      }).pipe(Effect.provide(scopeFor(workspaceDir))),
    ),
  );
