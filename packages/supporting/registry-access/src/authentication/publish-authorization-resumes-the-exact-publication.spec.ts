...
import { PublishAuthorizationPending, RegistryAccessFailed } from "./errors.js";
...
import { authFailureCategory, authFailureDetail } from "./test-support/test-helpers.js";
        ...
        if (status === "approved") {
          ...
          if (result._tag === "Failure") {
            if (status === "pending") {
              ...
              expect(result.failure).toBeInstanceOf(RegistryAccessFailed);
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(error).toBeInstanceOf(RegistryAccessFailed);
