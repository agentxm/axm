...
describe("resolveRepo", () => {
  ...
  describe("network errors", () => {
    it.effect("fails with AppError on network error", () =>
      Effect.gen(function* () {
        ...
        if (result._tag === "Failure") {
          ...
          expect(result.failure.what).toContain("Failed to check GitHub");
        }
      }),
    );

    it.effect("fails with AppError on subPath network error", () =>
      Effect.gen(function* () {
        let callCount = 0;
