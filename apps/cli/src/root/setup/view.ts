...
import { artifactChange, artifactChangeMark, count } from "../../screen/index.js";
...
export const setupAgentScanDoc = (scan: SetupAgentScan): Doc => [
  {
    ...
    aside: `found ${count(scan.detectedCount, "agent")}`,
  },
...
export const setupResultDoc = (result: SetupOutcome, options: SetupResultOptions): Doc => {
  ...
  switch (result.status) {
    case "cancelled":
      return [
        { _tag: "headline", tone: "warn", text: "Setup cancelled", aside: "nothing was changed" },
      ];
      ...
      return [
        {
          ...
          aside: joined([previewDefaultPhrase(result), "nothing was written"]),
        },
        ...next,
      ];
    case "initialized": {
