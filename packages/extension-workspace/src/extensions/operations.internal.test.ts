...
import { CoupledDependencyFailure } from "../extension-workspace/errors.js";
import { SourceAuthorityBlocked } from "./errors.js";
...
describe("buildInstallOperation", () => {
  ...
  it.effect("rejects installing over a workspace source before materialization", () =>
    Effect.gen(function* () {
      ...
      const result = yield* operation.run.pipe(
        Effect.match({
  ...
  it.effect("preflights materializability before scaffolding", () =>
    Effect.gen(function* () {
      ...
      const manager = {
        ...
        materializeInstall: () => Effect.void,
        listMaterializable: () =>
          Effect.fail(new CoupledDependencyFailure({ failure: "invalid pack" })),
        materializeUninstall: () => Effect.void,
        materializeDeactivate: () => Effect.void,
        upsertSettingsEntry: () => Effect.void,
        removeSettingsEntry: () => Effect.void,
  ...
  it.effect.each(["scaffold", "mark-authored", "resolve", "materialize", "commit"] as const)(
    ...
    (failureAt) =>
      Effect.gen(function* () {
        ...
        let listCalls = 0;
        const fail = () => Effect.fail(new CoupledDependencyFailure({ failure: failureAt }));
        const manager = {
          type: "skill",
    ...
    Effect.gen(function* () {
      ...
      const manager = {
        ...
        listMaterializable: () => {
          ...
          return listCalls === 1
            ? Effect.succeed([ref])
            : Effect.fail(new CoupledDependencyFailure({ failure: "unexpected preflight" }));
        },
        materializeInstall: () => Effect.void,
        materializeUninstall: () => Effect.void,
        materializeDeactivate: () => Effect.void,
  ...
  it.effect.each(["materialize", "settings", "lock", "validate"] as const)(
    ...
    (failureAt) =>
      Effect.gen(function* () {
        ...
        yield* Effect.flip(operation.run);
