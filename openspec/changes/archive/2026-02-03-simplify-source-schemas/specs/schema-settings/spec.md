...
### Requirement: SkillSettingsEntry union type

**Reason**: Replaced by simple source strings. Object variants (`_tag: "GitHub"`, `_tag: "Local"`) are no longer needed.

**Migration**: Convert object entries to source strings:

- `{ "_tag": "GitHub", "owner": "x", "repo": "y" }` → `"github:x/y"`
- `{ "_tag": "Local", "path": "./p" }` → `"local:./p"`
