...
### Requirement: Registry adapter translates generated client to RegistryClient interface

The registry adapter SHALL implement the `RegistryClient` interface by delegating HTTP transport to the generated registry client (`registry-client.ts`) and mapping all errors to `AppError` with per-operation error codes.

#### Scenario: Adapter constructed with base URL and HttpClient

- **WHEN** `createRemoteRegistryAdapter(baseUrl, httpClient)` is called
...
- **THEN** the adapter SHALL call the generated `ExtensionsGet` operation
...
- **THEN** the adapter SHALL fail with `AppError` code `REGISTRY_REMOTE_DISCOVERY_NETWORK_ERROR`
...
- **THEN** the adapter SHALL fail with `AppError` code `AUTH_UNAUTHENTICATED`
- **WHEN** `getExtensionIndex` encounters a 403 `RegistryClientError`
- **THEN** the adapter SHALL fail with `AppError` code `AUTH_UNAUTHORIZED` with required scope details

#### Scenario: getExtensionIndex maps decode errors

- **WHEN** `getExtensionIndex` encounters a `SchemaError`
- **THEN** the adapter SHALL fail with `AppError` code `REGISTRY_REMOTE_DISCOVERY_INVALID_RESPONSE`
...
- **THEN** the adapter SHALL call `ExtensionsListByProfile` when types is empty
...
- **THEN** the adapter SHALL fail with `AUTH_UNAUTHENTICATED`
- **WHEN** it encounters a `RegistryClientError` with status 403
- **THEN** the adapter SHALL fail with `AUTH_UNAUTHORIZED`
- **WHEN** it encounters any other `RegistryClientError`
- **THEN** the adapter SHALL fail with `REGISTRY_REMOTE_DISCOVERY_FAILED`
- **WHEN** it encounters a `SchemaError`
- **THEN** the adapter SHALL fail with `REGISTRY_REMOTE_DISCOVERY_INVALID_RESPONSE`

#### Scenario: profileExists delegates to ExtensionsListByProfile

- **WHEN** `profileExists` is called with a handle
- **THEN** the adapter SHALL call the generated `ExtensionsListByProfile` operation
...
- **THEN** the adapter SHALL fail with `REGISTRY_REMOTE_NAMESPACE_CHECK_NETWORK_ERROR`
- **WHEN** it encounters an unexpected status
- **THEN** the adapter SHALL fail with `REGISTRY_REMOTE_NAMESPACE_CHECK_FAILED`
- **WHEN** it encounters a `SchemaError`
- **THEN** the adapter SHALL fail with `REGISTRY_REMOTE_INVALID_RESPONSE`
...
- **THEN** the adapter SHALL first call `ExtensionsGet` to fetch the extension index
...
- **THEN** the adapter SHALL fail with `REGISTRY_REMOTE_PACKAGE_FETCH_NETWORK_ERROR`
- **WHEN** the index returns 404
- **THEN** the adapter SHALL fail with `REGISTRY_REMOTE_PACKAGE_NOT_FOUND`
- **WHEN** the archive returns 404
- **THEN** the adapter SHALL fail with `REGISTRY_REMOTE_PACKAGE_NOT_FOUND`
- **WHEN** the requested version is not found in the index
- **THEN** the adapter SHALL fail with `REGISTRY_REMOTE_VERSION_NOT_FOUND`
- **WHEN** any other unexpected status is returned
- **THEN** the adapter SHALL fail with `REGISTRY_REMOTE_PACKAGE_FETCH_FAILED`

#### Scenario: extensionExists delegates to ExtensionsHead

- **WHEN** `extensionExists` is called with handle, type, and name
- **THEN** the adapter SHALL call the generated `ExtensionsHead` operation
...
- **THEN** the adapter SHALL fail with `REGISTRY_REMOTE_EXTENSION_CHECK_NETWORK_ERROR`
- **WHEN** it encounters a 401
- **THEN** the adapter SHALL fail with `AUTH_UNAUTHENTICATED`
- **WHEN** it encounters a 403
- **THEN** the adapter SHALL fail with `AUTH_UNAUTHORIZED`
- **WHEN** it encounters any other unexpected status
- **THEN** the adapter SHALL fail with `REGISTRY_REMOTE_EXTENSION_CHECK_FAILED`
...
- **THEN** the adapter SHALL call the generated `ExtensionsPublishVersion` operation with multipart/form-data payload
- **AND** SHALL return `{published: true}` on 200 or 201

### Requirement: Registry adapter maps publish errors via typed RFC 9457 schemas

The registry adapter SHALL map `ExtensionsPublishVersion` error responses to specific `AppError` codes using the generated client's typed error schemas, replacing the hand-written `mapProblemDetailToAppError` raw JSON extraction.

#### Scenario: Publish conflict

- **WHEN** `publishExtension` encounters a 409 `RegistryClientError` with cause code `publish_conflict`
- **THEN** the adapter SHALL fail with `REGISTRY_PUBLISH_CONFLICT`
...
- **THEN** the adapter SHALL fail with `REGISTRY_PUBLISH_INVALID_ARCHIVE`
...
- **THEN** the adapter SHALL fail with `REGISTRY_PUBLISH_TOO_LARGE`
...
- **THEN** the adapter SHALL fail with `REGISTRY_PUBLISH_INTEGRITY_MISMATCH`
...
- **THEN** the adapter SHALL fail with `REGISTRY_PUBLISH_MANIFEST_INVALID`

#### Scenario: Rate limited

- **WHEN** `publishExtension` encounters a 429 `RegistryClientError` with cause code `throttled`
- **THEN** the adapter SHALL fail with `REGISTRY_PUBLISH_THROTTLED`
...
- **THEN** the adapter SHALL fail with `REGISTRY_PUBLISH_QUOTA_EXCEEDED`
...
- **THEN** the adapter SHALL fail with `REGISTRY_PUBLISH_TYPE_NOT_SUPPORTED`

#### Scenario: Publishing disabled

- **WHEN** `publishExtension` encounters a 503 `RegistryClientError` with cause code `publish_disabled`
- **THEN** the adapter SHALL fail with `REGISTRY_PUBLISH_DISABLED`
...
- **THEN** the adapter SHALL fail with `REGISTRY_PUBLISH_NETWORK_ERROR`
...
- **THEN** the adapter SHALL fail with `REGISTRY_PUBLISH_FAILED`

### Requirement: Auth adapter translates generated client to AuthClientService interface

The auth adapter SHALL implement the `AuthClientService` interface by delegating HTTP transport to the generated registry client's auth operations. The `registryUrl` is provided at construction time, not per-call.

#### Scenario: Adapter constructed with registry URL and HttpClient

- **WHEN** `createAuthAdapter(registryUrl, httpClient)` is called
...
- **THEN** the adapter SHALL call the generated `AuthIssueDeviceCode` with `client_id` and `scope` as form-urlencoded payload
...
- **THEN** the adapter SHALL fail with `AUTH_LOGIN_FAILED` with registry URL context

#### Scenario: pollDeviceToken implements RFC 8628 polling loop

- **WHEN** `pollDeviceToken(deviceCode, interval)` is called
- **THEN** the adapter SHALL repeatedly call the generated `AuthExchangeDeviceCode` at the specified interval
...
- **THEN** the adapter SHALL fail with `AUTH_SERVER_ERROR`

### Requirement: Telemetry adapter provides transport for generated telemetry client

The telemetry adapter SHALL provide a thin HTTP transport layer using the generated telemetry client. Metadata enrichment, mode gating, and fire-and-forget wrapping remain in `makeTelemetryClient`.

#### Scenario: Adapter constructed with HttpClient and base URL

- **WHEN** `createTelemetryAdapter(httpClient, baseUrl)` is called
...
- **THEN** the adapter SHALL call the generated `EventsIngest` operation
...
- **THEN** the adapter SHALL call the generated `ErrorsIngest` operation
...
- **THEN** the adapter SHALL silently discard the error
- **AND** SHALL NOT propagate it to callers

### Requirement: Network diagnostics preserved in adapters

The registry and auth adapters SHALL preserve the network diagnostic capabilities from the hand-written clients, including localhost+HTTPS detection and user-facing connection guidance.
...
- **THEN** the adapter SHALL include a diagnostic suggesting HTTP instead of HTTPS
...
The adapters SHALL share common error mapping helpers for consistent `AppError` construction across registry and auth operations.

#### Scenario: Auth error helpers reusable

- **WHEN** a 401 `RegistryClientError` is encountered in any adapter
...
- **WHEN** a 403 `RegistryClientError` is encountered in any adapter
