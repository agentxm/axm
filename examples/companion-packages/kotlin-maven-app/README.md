...
`../kotlin-maven-lib/.axm/extensions/` are designed to operate on.
...
[`kotlin-maven-pawmatch-find-a-pet`](./.axm/extensions/@examples/skills/kotlin-maven-pawmatch-find-a-pet/src/SKILL.md),
...
```bash
mvn -q -f ../kotlin-maven-lib/pom.xml install
mvn -q package
...
`mvn install` in `../kotlin-maven-lib/`.
