...
```json
{
  "recommendedExtensions": ["@examples/packs/cocoapods-tinyflags@^0.1.0"]
}
```
...
A working consumer is in `../cocoapods-app/` (the `pawmatch` CLI).
...
`../cocoapods-app/` via `:path =>` so no Trunk push is required for the example
to be reproducible.
...
| Type     | FQN                                                  |
| -------- | ---------------------------------------------------- |
| Skill    | `@examples/skills/cocoapods-tinyflags-add-flag`      |
| Skill    | `@examples/skills/cocoapods-tinyflags-rollout-review`|
| Skill    | `@examples/skills/cocoapods-tinyflags-cleanup-flag`  |
| Subagent | `@examples/subagents/cocoapods-tinyflags-maintainer` |
| Pack     | `@examples/packs/cocoapods-tinyflags`                |
...
The `swift-lib/` example demonstrates the same idea for Swift Package Manager
