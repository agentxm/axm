...
`../dotnet-fsharp-nuget-lib/.axm/extensions/` are designed to operate on.

`pawmatch` is not packable — it exists to demonstrate consumption, not to
publish.

The app also ships its own companion AXM skill,
[`dotnet-fsharp-nuget-pawmatch-find-a-pet`](./.axm/extensions/@examples/skills/dotnet-fsharp-nuget-pawmatch-find-a-pet/src/SKILL.md),
...
```xml
<add key="local-fsharp-lib" value="../dotnet-fsharp-nuget-lib/artifacts" />
```

Run `dotnet pack` in `../dotnet-fsharp-nuget-lib/` first so the `.nupkg` is on disk.
