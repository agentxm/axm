...
description: The accepted decision to type check on the native TypeScript 7
  compiler while `typescript` resolves to the TypeScript 6 compatibility
  package, exiting at TypeScript 7.1.
status: stable
tags: [typescript, toolchain, dual-alias, decision]
sources:
...
workflow. The choice is recorded as deliberate in the maintainer-accepted
AGENTS.md TypeScript section; this record gives that accepted choice its
governed decision lifecycle and itself lands through the same authority.
...
Binding limitation: this decision produces one independently binding
limitation — the dual alias must not be collapsed to a single `typescript`
dependency before the exit condition. That obligation is admitted as the
companion constraint Requirement
[AXM-REQ-0019, Dual TypeScript alias retained until exit](/system/requirements/constraint/dual-typescript-alias-retained-until-exit.md),
which lists this record among its `requirement_sources`; this record owns the
choice, rationale, and exit condition, not the obligation itself.
...
TypeScript dependency; the superseding record or change retires the companion
constraint Requirement through its lifecycle.
...
Per the general decision policy, also reconsider when a material assumption is
invalidated, evidence contradicts an intended consequence, or the tradeoff no
longer fits AXM's lifecycle or assurance posture. When superseded, this record
remains reachable and links its replacement.
