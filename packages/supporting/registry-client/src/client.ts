...
interface GetExtensionPackageBaseArgs {
  ...
  /**
   * Observes archive bytes as they arrive, and reports zero bytes when a
   * later request attempt begins. Callers throttle chunk measurements before
...
export interface RegistryClient {
  ...
  readonly ownerExists: (
    owner: Handle,
  ) => Effect.Effect<OwnerExistsResponse, RegistryClientFailure>;
  ...
  ) => Effect.Effect<VisibilityEvaluation, RegistryClientFailure>;
  readonly updateExtensionVisibility: (
    args: UpdateExtensionVisibilityArgs,
  ) => Effect.Effect<VisibilityMutationResult, RegistryClientFailure>;
