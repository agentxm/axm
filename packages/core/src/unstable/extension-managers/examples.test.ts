...
const EXTENSION_MANAGERS = import.meta.dirname;
...
describe("example files", () => {
  ...
  it("axm-skill.example.json conforms to SkillManifestSchema", () => {
    const example = readJsonFile(path.join(EXTENSION_MANAGERS, "skills/axm-skill.example.json"));
  ...
  it("axm-command.example.json conforms to CommandManifestSchema", () => {
    const example = readJsonFile(
      path.join(EXTENSION_MANAGERS, "commands/axm-command.example.json"),
    );
  ...
  it("axm-mcp-server.example.json conforms to McpServerManifestSchema", () => {
    const example = readJsonFile(
      path.join(EXTENSION_MANAGERS, "mcp-servers/axm-mcp-server.example.json"),
    );
  ...
  it("axm-pack.example.json conforms to PackManifestSchema", () => {
    const example = readJsonFile(path.join(EXTENSION_MANAGERS, "packs/axm-pack.example.json"));
