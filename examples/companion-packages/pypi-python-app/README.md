...
`../pypi-python-lib/.axm/extensions/` are designed to operate on.

`pawmatch` is not packable — it exists to demonstrate consumption, not to
publish.

The app also ships its own companion AXM skill,
[`pypi-python-pawmatch-find-a-pet`](./.axm/extensions/@examples/skills/pypi-python-pawmatch-find-a-pet/src/SKILL.md),
...
```bash
python -m venv .venv
.venv/bin/pip install -e ../pypi-python-lib -e . pytest
...
[tool.uv.sources]
agentxm-example-tinyflags = { path = "../pypi-python-lib", editable = true }
```
...
from the public index. The sibling `../pypi-python-lib/` is the source of
