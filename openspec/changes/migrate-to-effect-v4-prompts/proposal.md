...
- **CLI spike prompt commands migrate to Effect v4 `Prompt`** — All 10 existing prompt demo commands (`text`, `password`, `confirm`, `select`, `multiselect`, `group-multiselect`, `select-key`, `autocomplete`, `autocomplete-multiselect`, `path`) switch from `CliPrompt` service calls to direct `yield* Prompt.xxx(...)` usage (Prompt is Yieldable).
