...
describe("httpStatusToCategory", () => {
  ...
  ])("maps HTTP %s with code %s to %s", (status, problemCode, category) => {
    expect(httpStatusToCategory(status, problemCode)).toBe(category);
  });
});

describe("registryErrorToProblem", () => {
  it("uses the HTTP status when the problem document disagrees and retains the body", () => {
    ...
    const error = registryErrorToProblem(body, responseFor(503), { cause });
  ...
  it.each(["gateway unavailable", ["unexpected"], null])(
    ...
    (body) => {
      ...
      const error = registryErrorToProblem(body, responseFor(502), { cause });
  ...
  it("retains the generated client error as the cause", () => {
    ...
    const error = registryClientErrorToProblem(generated);
