...
import { allCatalogRuleIds } from "@agentxm/client-core/unstable/lint";
import { LOCKFILE_VERSION, LockfileSchema } from "@agentxm/client-core/unstable/lockfile";
import { SETTINGS_KEY_ORDER, SettingsSchema } from "@agentxm/client-core/unstable/settings";
...
const readPublishedSchema = (name: string): Record<string, unknown> => {
  const loaded: unknown = requireFromSpec(
    `@agentxm/client-core/unstable/site-content/schemas/${name}`,
  );
