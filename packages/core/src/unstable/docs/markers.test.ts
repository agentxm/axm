...
describe("file region markers", () => {
  it("uses markdown HTML comments", () => {
    ...
    const marker = serializeRegionMarker(
      { kind: "start", region: "toc", ext: "@acme/docs/readme" },
      style.value,
    );

    expect(marker).toBe("<!-- axm:start region=toc ext=@acme/docs/readme -->");
  ...
  it("replaces existing managed regions", () => {
    ...
    const original = [
      "# Project",
      "<!-- axm:start region=toc ext=@acme/docs/readme -->",
      "old",
      "<!-- axm:end region=toc ext=@acme/docs/readme -->",
      "tail",
    ].join("\n");

    const replaced = replaceManagedRegion({
      content: original,
      marker: { region: "toc", ext: "@acme/docs/readme" },
      rendered: "new",
      style: style.value,
    });
  ...
  it("strips existing managed regions", () => {
    ...
    const stripped = stripManagedRegion(
      original,
      { region: "toc", ext: "@acme/docs/readme" },
      style.value,
    );
