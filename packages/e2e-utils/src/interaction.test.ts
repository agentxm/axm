...
import { isAgent, isCI, isHumanInteractive, isInteractive } from "./interaction.ts";
...
describe("isInteractive", () => {
  ...
  it("returns true for agent sessions (does not check agent env)", () => {
    expect(isInteractive({ isTTY: true, env: { CODEX_CI: "1" } })).toBe(true);
  });
