...
describe("add.handler", () => {
  ...
  describe("source parsing", () => {
    it.effect("fails with AddError for invalid source format", () =>
      withTestLayer(
        Effect.gen(function* () {
          initializeAxm();
          const args: AddArgs = {
          ...
          const error = yield* handleAdd(args).pipe(Effect.flip);

          expect(error._tag).toBe("AddError");
          expect((error as AddError).message).toContain("Invalid source");
        }),
      ),
    );
    ...
    it.effect("handles empty source directory with error and recovery guidance", () =>
      withTestLayer(
        Effect.gen(function* () {
          ...
          expect(error._tag).toBe("AddError");
          expect((error as AddError).message).toContain("No skills found");
          expect((error as AddError).message).toContain(sourceDir);
          expect((error as AddError).message).toContain("SKILL.md");
        }),
      ),
    );
  ...
  describe("error scenarios", () => {
    it.effect("returns AddError with descriptive message for parsing errors", () =>
      withTestLayer(
        Effect.gen(function* () {
          initializeAxm();
          ...
          expect(error._tag).toBe("AddError");
          expect((error as AddError).message).toBeTruthy();
        }),
      ),
    );
    ...
    it.effect("invalid source error suggests valid source formats", () =>
      withTestLayer(
        Effect.gen(function* () {
          ...
          expect(error._tag).toBe("AddError");
          const message = (error as AddError).message;
  ...
  describe("non-TTY scenarios", () => {
    ...
    describe("agent selection in non-interactive mode", () => {
      it.effect("fails with AddError when stdin is not TTY and no --yes/--agent flag", () =>
        withTestLayer(
          Effect.gen(function* () {
            ...
            expect(error._tag).toBe("AddError");
            expect((error as AddError).message).toContain("Cannot prompt for agent selection");
            expect((error as AddError).message).toContain("stdin is not a TTY");
            expect((error as AddError).message).toContain("--yes");
            expect((error as AddError).message).toContain("--all");
            expect((error as AddError).message).toContain("--non-interactive");
          }),
        ),
      );
    ...
    describe("skill selection in non-interactive mode", () => {
      it.effect("fails with AddError when stdin is not TTY and no --all/--skill flag", () =>
        withTestLayer(
          Effect.gen(function* () {
            isInteractiveMock.mockReturnValue(false);
            const source = createSkillSource([{ name: "commit" }]);
            ...
            expect(error._tag).toBe("AddError");
            expect((error as AddError).message).toContain("Cannot prompt for skill selection");
            expect((error as AddError).message).toContain("stdin is not a TTY");
          }),
        ),
      );
    ...
    describe("confirmation in non-interactive mode", () => {
      it.effect("fails with AddError when stdin is not TTY and confirmation needed", () =>
        withTestLayer(
          Effect.gen(function* () {
            isInteractiveMock.mockReturnValue(false);
            const source = createSkillSource([{ name: "commit" }]);
            ...
            expect(error._tag).toBe("AddError");
            expect((error as AddError).message).toContain("Cannot prompt for confirmation");
            expect((error as AddError).message).toContain("stdin is not a TTY");
          }),
        ),
      );
