...
- **WHEN** `classifyError` receives a `AppError | PromptCancelled` value
...
**Reason**: Handlers no longer convert domain errors to AppError. Errors arrive as AppError from the source, so handler-level mapError wrappers are unnecessary.
**Migration**: Remove mapError wrappers from handlers. Ensure all service and utility functions create AppError directly.
...
**Migration**: AppError already uses `cause: unknown`. No action needed.
