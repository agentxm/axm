...
describe("makeMemoryTransitionLockWorld", () => {
  it.effect("reports contention under virtual time and releases on interruption", () =>
    Effect.gen(function* () {
      ...
      expect(Option.isSome(yield* first.held("/workspace"))).toBe(true);
      expect(Option.isNone(yield* second.held("/workspace"))).toBe(true);
      ...
      expect(Option.isNone(yield* first.held("/workspace"))).toBe(true);
      yield* Effect.scoped(
