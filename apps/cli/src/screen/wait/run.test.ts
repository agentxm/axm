...
describe("runWait", () => {
  ...
  it.effect("reports browser and clipboard failures without ending the wait", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness;
      yield* Queue.offerAll(harness.keys, [press("o"), press("c"), press("escape")]);

      yield* runWait(
        view,
        Effect.never,
        { open: Effect.succeed(false), copy: Effect.succeed(false) },
        harness.terminal,
        harness.surface,
      ).pipe(Effect.flip);

      const transcript = harness.transcript
        .flatMap((doc) => paintText(doc, { width: 80, colors: false }))
        .join("\n");
      expect(transcript).toContain("Could not open the browser. Use the URL above.");
      expect(transcript).toContain("Could not copy to the clipboard. Copy the value above.");
    }),
  );
