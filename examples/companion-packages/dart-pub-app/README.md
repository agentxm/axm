...
`../dart-pub-lib/.axm/extensions/` are designed to operate on.

`pawmatch` is not packable — it exists to demonstrate consumption, not to
publish.

The app also ships its own companion AXM skill,
[`dart-pub-pawmatch-find-a-pet`](./.axm/extensions/@examples/skills/dart-pub-pawmatch-find-a-pet/src/SKILL.md),
