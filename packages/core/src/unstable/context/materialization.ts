/**
 * context package materialization helpers.
 *
...
const resolvePayloadPath = (packageRoot: string, payloadPath: string) =>
  Effect.gen(function* () {
    ...
    if (isEscapingRelative(path, srcRoot, resolved)) {
      return yield* failValidation(`context package payload path escapes src/: ${payloadPath}`);
    }
    return resolved;
  });
...
const resolveWorkspaceTarget = (workspaceRoot: string, target: string) =>
  Effect.gen(function* () {
    ...
    if (Option.isNone(relative)) {
      return yield* failValidation(`context package target escapes workspace: ${target}`);
    }
    return {
  ...
  Effect.gen(function* () {
    ...
    return yield* fs.readFileString(resolved).pipe(
      Effect.mapError((error) =>
        makeAppError({
          code: "internal",
          detail: `Failed to read context payload: ${payloadPath}`,
          cause: error,
        }),
      ),
    );
  ...
  Effect.gen(function* () {
    ...
    yield* fs.writeFileString(absoluteTarget, content).pipe(
      Effect.mapError((error) =>
        makeAppError({
          code: "internal",
          detail: `Failed to write context target: ${absoluteTarget}`,
          cause: error,
        }),
      ),
    );
