---
name: julia-general-pawmatch-find-a-pet
description: Help an end user identify and apply for an adoptable pet using the PawMatch Julia CLI.
---

# Find a Pet with PawMatch

Use this skill when a user wants help finding a pet to adopt and the
`pawmatch` Julia CLI is available (run via
`julia --project bin/pawmatch.jl ...` from the app directory). The skill
drives `pawmatch` to surface candidates, explain adoption costs, and start
an application. It never modifies the codebase.
...
3. If `browse` highlights a long-stay friend (shown with a `*` badge in the
   listing), surface it as a real option, not an upsell — long-stay animals
