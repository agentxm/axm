---
name: swift-spm-pawmatch-find-a-pet
description: Help an end user identify and apply for an adoptable pet using the PawMatch Swift CLI.
---

# Find a Pet with PawMatch

Use this skill when a user wants help finding a pet to adopt and the
`pawmatch` Swift CLI is available (either as a built binary on `$PATH` or via
`swift run pawmatch` from this package). The skill drives `pawmatch` to
surface candidates, explain adoption costs, and start an application. It
...
6. When the user is ready, run `pawmatch apply <slug>` and walk them through
   the next steps — counselor review, meet-and-greet, 48-hour reflection, and
   possible two-week follow-up.

## Tone

- Match over transact. Adoption is a conversation, not a one-click checkout.
- Be honest about constraints (no-cats, training needs, bonded pairs). Do not
  hide them.
...
- Every recommendation comes from actual `pawmatch` output — no fabricated
  pets.
