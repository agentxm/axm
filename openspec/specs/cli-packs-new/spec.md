...
The pack name SHALL be scoped using the workspace's configured profile (from settings.json) unless overridden with `--profile`.
...
- **THEN** the command fails with a `AppError` indicating a profile is required
