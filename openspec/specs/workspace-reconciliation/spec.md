...
### Requirement: Lint unmanaged skill detection uses the workspace classifier

The `skills-managed` lint rule SHALL use the workspace classifier to identify unmanaged skills instead of inline per-agent detection logic. An artifact is unmanaged if and only if the classifier classifies it as `lifecycle: "unmanaged"`.
...
- **THEN** the lint rule SHALL report an unmanaged-skill finding for `orphaned-tool`
...
- **THEN** the lint rule SHALL report an unmanaged-skill finding for `old-shared`
...
- **THEN** the lint rule SHALL NOT report an unmanaged-skill finding for `active-shared`

### Requirement: Lint unmanaged skill findings suggest axm prune

Lint advisory messages for unmanaged skill artifacts SHALL suggest `axm prune` or `axm skills prune <name>` as the remediation action.

#### Scenario: Advisory message references prune command

- **WHEN** the lint rule reports an unmanaged-skill finding for `legacy-tool`
