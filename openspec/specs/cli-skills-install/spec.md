...
Installs skills from local paths, GitHub/GitLab repositories, or direct URLs into project or global scope.

## Requirements

### Requirement: Install Command Basic Invocation

The CLI SHALL provide an `install` sub-command under `skills` that installs skills from a source.

#### Scenario: Install command with GitHub shorthand

- **WHEN** the user runs `axm skills install owner/repo`
- **THEN** the CLI clones the repository, discovers available skills, prompts for selection, and installs selected skills

#### Scenario: Install command with GitHub URL

- **WHEN** the user runs `axm skills install https://github.com/owner/repo`
- **THEN** the CLI clones the repository, discovers available skills, prompts for selection, and installs selected skills

#### Scenario: Install command with local path

- **WHEN** the user runs `axm skills install ./path/to/skills`
...
- **WHEN** the user runs `axm skills install owner/repo` without flags
- **THEN** skills are installed to `.axm/skills/` in the current project

#### Scenario: Global installation with --global flag

- **WHEN** the user runs `axm skills install owner/repo --global`
...
- **WHEN** the user runs `axm skills install owner/repo` and multiple agents are detected
...
- **WHEN** the user runs `axm skills install owner/repo --agent claude-code cursor`
- **THEN** the CLI installs skills only to the specified agents without prompting

#### Scenario: Agent detection

- **WHEN** the CLI starts the install flow
...
- **WHEN** the user runs `axm skills install owner/repo --skill pr-review commit`
- **THEN** the CLI installs only the specified skills without prompting

#### Scenario: List available skills with --list flag

- **WHEN** the user runs `axm skills install owner/repo --list`
...
- **WHEN** the user runs `axm skills install owner/repo --yes`
- **THEN** the CLI uses default selections without interactive prompts

#### Scenario: Install all with --all flag

- **WHEN** the user runs `axm skills install owner/repo --all`
