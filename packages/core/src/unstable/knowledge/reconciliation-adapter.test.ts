...
import { knowledgeReconciliationAdapter } from "./reconciliation-adapter.js";
...
describe("knowledge reconciliation adapter", () => {
  let tempDir: string;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "axm-knowledge-reconcile-"));
  });
  ...
  it.effect("requires canonical registry metadata instead of reconstructing it from disk", () =>
    Effect.gen(function* () {
      const snapshot = yield* snapshotFor({
        skills: {},
        knowledge: {
          baseline: { source: "@acme/knowledge/handbook@^1", enabled: true },
        },
      });

      expect(Object.keys(snapshot.lockfile.knowledge ?? {})).toEqual([]);
  ...
  it.effect("produces no lock entry when no knowledge bundle is declared", () =>
    Effect.gen(function* () {
      const snapshot = yield* snapshotFor({ skills: {}, knowledge: {} });

      expect(snapshot.lockfile.knowledge ?? {}).toEqual({});
      expect(snapshot.unresolved).toEqual([]);
    }),
  );
});
