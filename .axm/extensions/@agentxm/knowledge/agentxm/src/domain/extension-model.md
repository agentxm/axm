...
| Library   | A live, unordered registry collection of extension identities. A Library is _not_ an extension type: it has no manifest, archive, version, or publish step, and it is never an install target. |
...
installed: an entry is _configured_ (explicit settings entry), _implicit_
(desired only as a pack member), or _unmanaged_ (present and usable but
