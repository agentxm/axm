...
- [ ] `packages/cli/src/commands/skills/install/` exists with all files from add/
- [ ] `packages/cli/src/commands/skills/add/` no longer exists
- [ ] Git shows rename (not delete+add) in status
...
- [ ] `packages/cli/src/commands/skills/command.ts` registers "install" sub-command
- [ ] No references to "add" remain in command registration
...
- [ ] All function/variable names using "add" updated to "install" where appropriate
- [ ] All user-facing strings say "install" instead of "add"
- [ ] `pnpm typecheck` passes
- [ ] `pnpm lint` passes
...
- [ ] `packages/cli/e2e/skills-install.test.ts` exists
- [ ] `packages/cli/e2e/skills-add.test.ts` no longer exists
- [ ] Test descriptions reference "install" not "add"
- [ ] Test commands use `axm skills install` not `axm skills add`
...
- [ ] All test descriptions reference "install" not "add"
- [ ] `pnpm test` passes
...
- [ ] `pnpm build` succeeds
- [ ] `axm skills install --help` displays help for install command
- [ ] `axm skills add` shows error or unknown command (not silently accepted)
