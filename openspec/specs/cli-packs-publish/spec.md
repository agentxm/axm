...
- **THEN** the pack directory is validated (must contain `pack.json`)
...
- **WHEN** user runs `axm packs publish @acme/frontend-tools`
- **AND** `pack.json` does not exist in the pack directory
...
- **WHEN** the pack directory contains `pack.json` and `README.md`
