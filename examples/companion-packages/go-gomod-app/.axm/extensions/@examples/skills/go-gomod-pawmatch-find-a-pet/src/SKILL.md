---
name: go-gomod-pawmatch-find-a-pet
...
`pawmatch` Go CLI is available (either as a built binary on `$PATH` or via
`go run ./cmd/pawmatch` from this repository). The skill drives `pawmatch`
to surface candidates, explain adoption costs, and start an application.
It never modifies the codebase.
