...
`WorkspaceContext` is the read-only, decode-tolerant projection over the workspace state layers (declared, resolved, actual). It is the intended primary CLI read model for workspace state, exposed as a single `Context.Service` with a scope-first API. Each cell loads independently, exposes per-source tagged errors, and distinguishes absent state from invalid state. Actual entries preserve distinct same-name materializations and carry subject-specific detection origins.

> Historical note: this capability used the original `WorkspaceContext` name during design. The implemented API is now `WorkspaceReadModel`; the capability slug is preserved for traceability.
