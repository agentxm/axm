...
const decodeManifest = Schema.decodeUnknownEffect(ContextManifestSchema);
...
const toAppError = (error: AppError | unknown): AppError =>
  isAppError(error)
    ...
    : makeAppError({
        code: "internal",
        detail: "Failed to publish context package",
        cause: error,
      });

const publishContext = (args: {
...
> =>
  Effect.gen(function* () {
    ...
    if (parsed === undefined || parsed.type !== "context" || parsed.name === undefined) {
      return yield* makeAppError({
        code: "validation",
        detail: "Use @owner/context/name",
      });
    }
    ...
    const manifestPath = path.join(extensionDir, CONTEXT_MANIFEST_FILENAME);
      ...
      Effect.flatMap((content) =>
        Effect.try({
          try: (): unknown => JSON.parse(content),
            ...
            makeAppError({
              code: "validation",
              detail: `Invalid JSON in ${CONTEXT_MANIFEST_FILENAME}`,
              cause: error,
            }),
        }),
      ),
    );
    const manifest: ContextManifest = yield* decodeManifest(manifestJson).pipe(
      Effect.mapError((error) =>
        makeAppError({
          code: "validation",
          detail: `Invalid ${CONTEXT_MANIFEST_FILENAME}`,
          cause: error,
        }),
      ),
    );
...
export const handleContextPublish = Effect.fn("ContextPublish.handle")(function* (args: {
  readonly input: string;
  readonly registry: Option.Option<string>;
  ...
  const runPublishPlan = Effect.gen(function* () {
          ...
          steps: [
            {
              readiness: "ready",
              label: args.input,
              run: publishContext({ input: args.input, registry: args.registry }).pipe(
    ...
    yield* emitPlanResolutionResult("context.publish", resolution);
...
const publishConfig = {
  input: Argument.string("input").pipe(
    Argument.withDescription("context package FQN (@owner/context/name)"),
  ),
...
export const publishCommand = Command.make(
  ...
  ({ input, registry, scope, yes, force, preview }) => {
    const program = handleContextPublish({ input, registry, yes, force, preview }).pipe(
      withWorkspace(scope),
    );
    return program.pipe(Effect.provide(AuthLayer), withRuntime("context publish"));
  },
).pipe(
  withArgvTracking(publishConfig),
  Command.withDescription("Publish a context package"),
  Command.withExamples([
    {
      command: "axm context publish @acme/context/workspace-baseline",
      description: "Publish a context package",
    },
  ]),
);
