...
import { displayWidth, padDisplay, truncateLine } from "./width.js";
...
export interface Glyphs {
  ...
  /** Mark appended when authored text is shortened to the available width. */
 ...
 * preserve the same semantic roles; wider ASCII pick marks may shift option
 * text as the design's ASCII reference intentionally shows.
 */
...
const withAside = (
  ...
  if (visibleText(aside).length === 0) return lines;
...
const withTrailing = (
  ...
  if (visibleText(trailing).length === 0) return lines;
...
const paintTable = (
  ...
  if (layout._tag === "stacked") {
    ...
    return rows.flatMap((row, rowIndex) => {
      ...
      if (row.cells.every((cell) => visibleText(cell).length === 0)) return [];
...
const paintLedger = (
  ...
  return [
    ...(headerCells.some((header) => visibleText(header.text).length > 0)
    ...
    case "headline": {
      ...
      if (mark === undefined || node.verdict === true) {
        ...
        return node.aside === undefined
          ? lines
          : withTrailing(lines, joinedParts(node.aside, glyphs), style, content + 2);
      }
      ...
      return node.aside === undefined
        ? lines
        : withAside(lines, joinedParts(node.aside, glyphs), style);
    }
    case "paragraph":
    ...
    case "next":
      return [
        ...
        ...node.actions.flatMap((action) => {
          ...
          if (action.url !== undefined || visibleText(action.description).length === 0)
            return lines;
          return withTrailing(lines, action.description, style, actionStart, {
