...
- **THEN** the handler SHALL fail with an `AppError` indicating the skill was not found in configured entries
...
- **THEN** the handler SHALL fail with an `AppError` indicating name conflict
