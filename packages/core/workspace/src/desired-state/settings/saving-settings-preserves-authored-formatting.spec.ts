...
import { writeSettingsAtPath } from "./settings.js";
...
/**
 * A directory the writer can write into. The rule is about the text of one
 * file, so nothing else of a workspace is needed.
 */
const makeSettingsDirectory = () => {
  const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "axm-settings-format-")));
...
describe("Saving settings", () => {
  ...
  it.effect.each([
    ...
    Effect.gen(function* () {
      ...
      const settingsPath = path.join(directory.root, "axm.json");
      fs.writeFileSync(settingsPath, text);

      const authored: unknown = JSON.parse(text);
      const settings = yield* decodeSettings(authored);
      yield* writeSettingsAtPath(settingsPath, settings).pipe(Effect.provide(NodeServices.layer));
