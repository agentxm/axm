  ...
  describe("fork from installed skill", () => {
    it("forks an installed skill to an extension and publishes to registry", async () => {
      ...
      try {
        ...
        // Verify skill.json manifest was generated at extension root (not inside src/)
        const manifestPath = path.join(extensionDir, "skill.json");
        expect(fs.existsSync(manifestPath)).toBe(true);
        expect(fs.existsSync(path.join(extensionDir, "src", "skill.json"))).toBe(false);
    ...
    it("forks multiple skills via glob pattern", async () => {
      ...
      try {
        ...
        for (const skillName of ["my-skill", "another-skill"]) {
          ...
          expect(fs.existsSync(extensionDir)).toBe(true);
          expect(fs.existsSync(path.join(extensionDir, "skill.json"))).toBe(true);
    ...
    it("shows expanded available candidates for glob no-match", async () => {
      ...
      try {
        ...
        expect(output).toContain("NO_SKILLS_MATCHED");
        expect(output).toContain("No skills matched the given pattern");
      } finally {
        temp.cleanup();
        registryDir.cleanup();
      }
