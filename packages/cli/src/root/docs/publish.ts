...
import {
  ...
  DocsManifestSchema,
  type DocsManifest,
} from "@agentxm/client-core/unstable/docs";
...
const decodeManifest = Schema.decodeUnknownEffect(DocsManifestSchema);
...
const toAppError = (error: AppError | unknown): AppError =>
  isAppError(error)
    ...
    : makeAppError({
        code: "internal",
        detail: "Failed to publish docs package",
        cause: error,
      });

const publishDocs = (args: {
...
> =>
  Effect.gen(function* () {
    ...
    if (parsed === undefined || parsed.type !== "docs" || parsed.name === undefined) {
      return yield* makeAppError({
        code: "validation",
        detail: "Use @owner/docs/name",
      });
    }
    ...
    const manifestPath = path.join(extensionDir, DOCS_MANIFEST_FILENAME);
      ...
      Effect.flatMap((content) =>
        Effect.try({
          try: (): unknown => JSON.parse(content),
            ...
            makeAppError({
              code: "validation",
              detail: `Invalid JSON in ${DOCS_MANIFEST_FILENAME}`,
              cause: error,
            }),
        }),
      ),
    );
    const manifest: DocsManifest = yield* decodeManifest(manifestJson).pipe(
      Effect.mapError((error) =>
        makeAppError({
          code: "validation",
          detail: `Invalid ${DOCS_MANIFEST_FILENAME}`,
          cause: error,
        }),
      ),
    );
...
export const handleDocsPublish = Effect.fn("DocsPublish.handle")(function* (args: {
  readonly input: string;
  readonly registry: Option.Option<string>;
  ...
  const runPublishPlan = Effect.gen(function* () {
          ...
          steps: [
            {
              readiness: "ready",
              label: args.input,
              run: publishDocs({ input: args.input, registry: args.registry }).pipe(
    ...
    yield* emitPlanResolutionResult("docs.publish", resolution);
...
const publishConfig = {
  input: Argument.string("input").pipe(
    Argument.withDescription("docs package FQN (@owner/docs/name)"),
  ),
...
export const publishCommand = Command.make(
  ...
  ({ input, registry, scope, yes, force, preview }) => {
    const program = handleDocsPublish({ input, registry, yes, force, preview }).pipe(
      withWorkspace(scope),
    );
    return program.pipe(Effect.provide(AuthLayer), withRuntime("docs publish"));
  },
).pipe(
  withArgvTracking(publishConfig),
  Command.withDescription("Publish a docs package"),
  Command.withExamples([
    {
      command: "axm docs publish @acme/docs/workspace-baseline",
