...
const subPathNotFoundLayer = makeMockHttpLayer((url) => ({
  status: url.pathname.includes("/-/tree/") ? 404 : 200,
}));
...
describe("resolveRepo", () => {
    ...
    it.effect("resolves owner/repo to Some when repo exists", () =>
      Effect.gen(function* () {
        ...
        if (Option.isSome(result)) {
          expect(result.value.type).toBe("gitlab");
  ...
  describe("network errors", () => {
    it.effect("fails with a typed failure on network failure", () =>
      Effect.gen(function* () {
        ...
        expect(error.detail).toContain("Failed to check GitLab");
      }),
    );
  });
});
