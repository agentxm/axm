...
describe("Pack add preview purity", () => {
  ...
  /** A workspace with one installed Registry skill and one empty authored pack. */
  const authoredPackWorkspace = Effect.gen(function* () {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const failure = yield* handlePacksAdd({
          pack: "toolkit",
          extension: "@acme/skills/absent-skill",
          preview: true,
