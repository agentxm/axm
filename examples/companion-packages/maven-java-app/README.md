...
`../maven-java-lib/.axm/extensions/` are designed to operate on.

`pawmatch` is not publishable — it exists to demonstrate consumption, not to
publish.

The app also ships its own companion AXM skill,
[`maven-java-pawmatch-find-a-pet`](./.axm/extensions/@examples/skills/maven-java-pawmatch-find-a-pet/src/SKILL.md),
...
```bash
cd ../maven-java-lib && mvn -q install
cd ../maven-java-app
