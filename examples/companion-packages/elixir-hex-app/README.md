...
`../elixir-hex-lib/.axm/extensions/` are designed to operate on.

`pawmatch` is not publishable — it exists to demonstrate consumption, not to
publish.

The app also ships its own companion AXM skill,
[`elixir-hex-pawmatch-find-a-pet`](./.axm/extensions/@examples/skills/elixir-hex-pawmatch-find-a-pet/src/SKILL.md),
