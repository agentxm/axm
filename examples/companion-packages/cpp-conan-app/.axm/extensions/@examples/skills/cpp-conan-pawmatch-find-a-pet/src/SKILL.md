---
name: cpp-conan-pawmatch-find-a-pet
...
`pawmatch` C++ CLI is available (either as a built binary on `$PATH` or
through `cmake --build` from this repository). The skill drives `pawmatch`
