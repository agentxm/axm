...
import { NativeImportInvalid, NativeImportUnsupported } from "@agentxm/extension-workspace";
  ...
  it.effect("rejects managed AXM packages and directs callers to fork", () =>
    Effect.gen(function* () {
      ...
      if (Result.isFailure(result)) {
        expect(result.failure).toBeInstanceOf(NativeImportInvalid);
        expect(
          result.failure instanceof NativeImportInvalid ? result.failure.detail : "",
        ).toContain("use fork");
      }
    }),
  );
