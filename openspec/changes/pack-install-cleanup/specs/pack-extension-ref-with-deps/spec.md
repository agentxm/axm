...
Keys in each map SHALL be fully qualified names including the type segment (e.g., `@scope/skills/name`, `@scope/commands/name`, `@scope/mcp-servers/name`). Values SHALL be version constraints (e.g., `"^1.0.0"`, `"*"`).
...
Dependency keys SHALL use the format `@scope/<type-plural>/<name>` (e.g., `@acme/skills/code-review`, `@acme/commands/formatter`, `@acme/mcp-servers/db`). Values SHALL be the version constraint from the pack manifest.
...
- **WHEN** a dependency key does not match the `@scope/<type-plural>/<name>` format
