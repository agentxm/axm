...
import { buildPlan, getPlanSummary, planHasChanges, planToJson } from "./plan.js";
...
describe("planToJson", () => {
  it("returns empty steps array and zero summary for empty plan", () => {
    const plan = { steps: [] };
  ...
  it("serializes InstallSkill step with Option values as null", () => {
    const plan = {
      steps: [
        {
          _tag: "InstallSkill" as const,
  ...
  it("serializes UpdateSkill step with version information", () => {
    const plan = {
      steps: [
        {
          _tag: "UpdateSkill" as const,
  ...
  it("serializes UninstallSkill step", () => {
    const plan = {
      steps: [
        {
          _tag: "UninstallSkill" as const,
  ...
  it("serializes Local source correctly", () => {
    const plan = {
      steps: [
        {
          _tag: "InstallSkill" as const,
    ...
    expect(json.steps[0]?.source).toEqual({
      _tag: "Local",
      path: "/path/to/skill",
    });
  });

  it("serializes mixed plan with correct summary", () => {
    const plan = {
      steps: [
        {
          _tag: "InstallSkill" as const,
          skill: "install-skill",
          source: { _tag: "Local" as const, path: "/install" },
          version: Option.none(),
          gitTreeHash: Option.none(),
          agents: ["claude"],
        },
        {
          _tag: "UpdateSkill" as const,
          skill: "update-skill",
          source: { _tag: "Local" as const, path: "/update" },
          fromVersion: Option.none(),
          toVersion: Option.none(),
          fromHash: Option.none(),
          toHash: Option.none(),
